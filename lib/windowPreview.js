...
var WindowPreviewModule = class {
    ...
    update(reset) {
        this._moduleEnabled = opt.get('windowPreviewModule');
    ...
    _activateModule() {
        ...
        this._overrides.addOverride('WindowPreview', Ui.WindowPreview.WindowPreview.prototype, WindowPreviewCommon);
        ...
        if (opt.ALWAYS_ACTIVATE_SELECTED_WINDOW)
            Ui.WindowPreview.WINDOW_OVERLAY_IDLE_HIDE_TIMEOUT = 150;
    }

    _disableModule() {
        // If Ui.WindowPreview._init was injected by another extension (like Burn My Windows)
        ...
        const skipReset = Ui.WindowPreview.WindowPreview.prototype._init.toString().includes('fn.apply(this, args)');
...
const WindowPreviewCommon = {
    _init(metaWindow, workspace, overviewAdjustment) {
        ...
        Gi.Shell.WindowPreview.prototype._init.bind(this)({
            reactive: true,
            can_focus: true,
            accessible_role: Gi.Atk.Role.PUSH_BUTTON,
            offscreen_redirect: Gi.Clutter.OffscreenRedirect.AUTOMATIC_FOR_OPACITY,
        });

        const windowContainer = new Gi.Clutter.Actor({
            pivot_point: new Gi.Graphene.Point({ x: 0.5, y: 0.5 }),
        });
        ...
        // container
        windowContainer.layout_manager = new Gi.Shell.WindowPreviewLayout();
        ...
        let clickAction = new Gi.Clutter.ClickAction();
        clickAction.connect('clicked', act => {
            const button = act.get_button();
            if (button === Gi.Clutter.BUTTON_SECONDARY) {
                if (opt.WIN_PREVIEW_SEC_BTN_ACTION === 1) {
                    ...
                    return Gi.Clutter.EVENT_STOP;
                } else if (opt.WIN_PREVIEW_SEC_BTN_ACTION === 3) {
                    this._removeLaters();
                    Me.Modules.winTmbModule.createThumbnail(metaWindow);
                    return Gi.Clutter.EVENT_STOP;
                }
            } else if (button === Gi.Clutter.BUTTON_MIDDLE) {
                if (opt.WIN_PREVIEW_MID_BTN_ACTION === 1) {
                    ...
                    this._searchAppWindowsAction();
                    return Gi.Clutter.EVENT_STOP;
                } else if (opt.WIN_PREVIEW_MID_BTN_ACTION === 3) {
                    this._removeLaters();
                    Me.Modules.winTmbModule.createThumbnail(metaWindow);
                    return Gi.Clutter.EVENT_STOP;
                }
        ...
        if (this._onLongPress) {
            ...
            clickAction.connect('long-press', (action, actor, state) => {
                if (state === Gi.Clutter.LongPressState.ACTIVATE)
        ...
        this._draggable = Ui.DND.makeDraggable(this, {
        ...
        this._icon.add_constraint(new Gi.Clutter.BindConstraint({
            source: windowContainer,
            coordinate: Gi.Clutter.BindCoordinate.POSITION,
        }));
        this._icon.add_constraint(new Gi.Clutter.AlignConstraint({
            source: windowContainer,
            align_axis: Gi.Clutter.AlignAxis.X_AXIS,
            factor: 0.5,
        }));
        this._icon.add_constraint(new Gi.Clutter.AlignConstraint({
            source: windowContainer,
            align_axis: Gi.Clutter.AlignAxis.Y_AXIS,
            pivot_point: new Gi.Graphene.Point({ x: -1, y: ICON_OVERLAP }),
            factor: 1,
        }));

        if (opt.WINDOW_ICON_CLICK_ACTION) {
            const iconClickAction = new Gi.Clutter.ClickAction();
            iconClickAction.connect('clicked', act => {
                if (act.get_button() === Gi.Clutter.BUTTON_PRIMARY) {
                    if (opt.WINDOW_ICON_CLICK_ACTION === 1) {
                        this._searchAppWindowsAction();
                        return Gi.Clutter.EVENT_STOP;
                    } else if (opt.WINDOW_ICON_CLICK_ACTION === 2) {
                        this._removeLaters();
                        Me.Modules.winTmbModule.createThumbnail(metaWindow);
                        return Gi.Clutter.EVENT_STOP;
                    }
                } /* else if (act.get_button() === Gi.Clutter.BUTTON_SECONDARY) {
                    return Gi.Clutter.EVENT_STOP;
                }*/
        ...
        this._title.add_constraint(new Gi.Clutter.BindConstraint({
            source: windowContainer,
            coordinate: Gi.Clutter.BindCoordinate.X,
        }));
        ...
        this._title.add_constraint(new Gi.Clutter.BindConstraint({
            source: windowContainer,
            coordinate: Gi.Clutter.BindCoordinate.Y,
            offset,
        }));
        this._title.add_constraint(new Gi.Clutter.AlignConstraint({
            source: windowContainer,
            align_axis: Gi.Clutter.AlignAxis.X_AXIS,
            factor: 0.5,
        }));
        this._title.add_constraint(new Gi.Clutter.AlignConstraint({
            source: windowContainer,
            align_axis: Gi.Clutter.AlignAxis.Y_AXIS,
            pivot_point: new Gi.Graphene.Point({ x: -1, y: 0 }),
            factor: 1,
        }));
        this._title.clutter_text.ellipsize = Gi.Pango.EllipsizeMode.END;
        this.label_actor = this._title;
        this.metaWindow.connectObject(
            'notify::title', () => (this._title.text = this._getCaption()),
            this);

        const layout = Gi.Meta.prefs_get_button_layout();
        this._closeButtonSide =
            layout.left_buttons.includes(Gi.Meta.ButtonFunction.CLOSE)
                ? Gi.St.Side.LEFT : Gi.St.Side.RIGHT;
        if (Misc.shellVersion < 43) {
            this._closeButton = new Gi.St.Button({
                visible: false,
                style_class: 'window-close',
                child: new Gi.St.Icon({ icon_name: 'preview-close-symbolic' }),
            });
        } else {
            this._closeButton = new Gi.St.Button({
                visible: false,
        ...
        this._closeButton.add_constraint(new Gi.Clutter.BindConstraint({
            source: windowContainer,
            coordinate: Gi.Clutter.BindCoordinate.POSITION,
        }));
        this._closeButton.add_constraint(new Gi.Clutter.AlignConstraint({
            source: windowContainer,
            align_axis: Gi.Clutter.AlignAxis.X_AXIS,
            pivot_point: new Gi.Graphene.Point({ x: 0.5, y: -1 }),
            factor: this._closeButtonSide === Gi.St.Side.LEFT ? 0 : 1,
        }));
        this._closeButton.add_constraint(new Gi.Clutter.AlignConstraint({
            source: windowContainer,
            align_axis: Gi.Clutter.AlignAxis.Y_AXIS,
            pivot_point: new Gi.Graphene.Point({ x: -1, y: 0.5 }),
            factor: 0,
        }));
        ...
        if (Ui.Main.overview._overview._controls._stateAdjustment.value < 1) {
        ...
        if (opt.OVERVIEW_MODE === 1) {
            ...
            this._wsStateConId = this.connect('enter-event', () => {
                ...
                if (global.get_pointer()[0] === opt.showingPointerX || Ui.Main.overview._overview._controls._stateAdjustment.value < 1)
                    return;
    ...
    _removeLaters() {
        ...
        if (this._longPressLater) {
            if (Gi.Meta.later_remove)
                Gi.Meta.later_remove(this._longPressLater);
            else
    ...
    _updateIconScale() {
        ...
        if (!opt.WS_ANIMATION && (Ui.Main.overview._overview.controls._searchController.searchActive ||
            ((initialState === ControlsState.WINDOW_PICKER && finalState === ControlsState.APP_GRID) ||
             (initialState === ControlsState.APP_GRID && finalState === ControlsState.WINDOW_PICKER)))
        )
            return;
    ...
    hideOverlay(animate) {
        ...
        if (opt.ALWAYS_ACTIVATE_SELECTED_WINDOW && Ui.Main.overview._overview.controls._stateAdjustment.value < 1)
    ...
    _onDestroy() {
        ...
        if (this._longPressLater) {
            if (Gi.Meta.later_add)
                Gi.Meta.later_add(Gi.Meta.LaterType.BEFORE_REDRAW, this._longPressLater);
            else
                global.compositor.get_laters().add(Gi.Meta.LaterType.BEFORE_REDRAW, this._longPressLater);
            delete this._longPressLater;
        }

        if (this._idleHideOverlayId > 0) {
            Gi.GLib.source_remove(this._idleHideOverlayId);
            this._idleHideOverlayId = 0;
        }
