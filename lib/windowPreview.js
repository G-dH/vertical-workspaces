...
const WindowPreviewCommon = {
    _init(metaWindow, workspace, overviewAdjustment) {
        ...
        this._closeButton = new St.Button({
            visible: opt.SHOW_CLOSE_BUTTON === 2 && !(opt.OVERVIEW_MODE2 && !opt.WORKSPACE_MODE),
    ...
    showOverlay(animate) {
        ...
        const toShow = this._windowCanClose() && opt.SHOW_CLOSE_BUTTON === 1
            ? [this._closeButton]
            : [];
    ...
    hideOverlay(animate) {
        ...
        const toHide = opt.SHOW_CLOSE_BUTTON === 1
            ? [this._closeButton]
            : [];
