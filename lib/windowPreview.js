...
function update(reset) {
    ...
    if (reset && !WindowPreview.WindowPreview.prototype._init.toString().includes('fn.apply(this, args)'))
        return;
...
const WindowPreviewCommon = {
    ...
    after__init() {
        ...
        if (opt.WIN_TITLES_POSITION < 2) {
            ...
            const offset = scaleFactor * (opt.WIN_PREVIEW_ICON_SIZE * ICON_OVERLAP + 35);
            this._title.get_constraints()[1].offset = -offset;
        } else {
            const offset = scaleFactor * (opt.WIN_PREVIEW_ICON_SIZE * (1 - ICON_OVERLAP) + 4);
            this._title.get_constraints()[1].offset = offset;
        }
