...
export var WorkspaceModule = class {
    ...
    update(reset) {
        this._moduleEnabled = opt.get('workspaceModule');
    ...
    _activateModule() {
        ...
        this._overrides.addOverride('WorkspaceBackground', Ui.Workspace.WorkspaceBackground.prototype, WorkspaceBackground);

        // fix overlay base for Vertical Workspaces
        this._overrides.addOverride('WorkspaceLayout', Ui.Workspace.WorkspaceLayout.prototype, WorkspaceLayout);
    }
    ...
    _disableModule() {
        ...
        this._overrides = null;
        // Ui.Workspace.WINDOW_PREVIEW_MAXIMUM_SCALE = 0.95; // incompatible 45
...
const WorkspaceLayout = {
        ...
        if (opt.OVERVIEW_MODE === 1) {
            this._stateAdjustment.connect('notify::value', () => {
                ...
                if (scale !== this.WINDOW_PREVIEW_MAXIMUM_SCALE) {
                    ...
                    Ui.Workspace.WINDOW_PREVIEW_MAXIMUM_SCALE = scale;
    ...
    _adjustSpacingAndPadding(rowSpacing, colSpacing, containerBox) {
        ...
        if (containerBox) {
            ...
            const monitor = Ui.Main.layoutManager.monitors[this._monitorIndex];

            const bottomPoint = new Gi.Graphene.Point3D();
            if (vertical)
...
const WorkspaceBackground = {
    _updateBorderRadius(value = false) {
        ...
        backgroundContent.rounded_clip_radius =
            Misc.Util.lerp(0, cornerRadius, value);
    },
};
