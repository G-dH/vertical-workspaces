...
const WorkspaceCommon = {
    _init(metaWorkspace, monitorIndex, overviewAdjustment) {
        ...
        if (Clutter.ClickAction) {
            ...
            clickAction.connect('clicked', action => onWsClicked(action.get_button()));
        } else { // Since GNOME 49.rc
            clickAction = new Clutter.ClickGesture({
                required_button: Clutter.BUTTON_PRIMARY,
            });
            clickAction.connect('recognize', gesture => onWsClicked(gesture.get_button()));
        }
