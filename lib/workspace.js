...
const WorkspaceCommon = {
    _init(metaWorkspace, monitorIndex, overviewAdjustment) {
        ...
        clickAction.connect(clickedSignal,
            action => onWsClicked(action.get_button())
        );
...
const WorkspaceLayout = {
    ...
    vfunc_allocate(container, box) {
        ...
        const inSessionTransition =
            ...
            // Don't clip workspace on secondary monitors
        ...
        for (let i = 0; i < nSlots; i++) {
            ...
            if (layoutChanged && !Main.overview.animationInProgress) {
                const transition = this._animateAllocation(child, childBox);
                if (transition) {
