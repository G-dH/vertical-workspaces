...
function _newImageFromIconName(name) {
    return Gi.Gtk.Image.new_from_icon_name(name);
}
...
var ItemFactory = class ItemFactory {
    ...
    _connectDropDown(widget, key, variable, options) {
        ...
        for (let i = 0; i < options.length; i++) {
            ...
            model.append(new Gi.DropDownItem({ text, id }));
            if (id === currentValue)
    ...
    newLinkButton(uri) {
        const linkBtn = new Gi.Gtk.LinkButton({
            label: Misc.shellVersion < 42 ? 'Click Me!' : '',
            uri,
            halign: Gi.Gtk.Align.END,
            valign: Gi.Gtk.Align.CENTER,
            hexpand: true,
...
/* var LegacyPrefs = class {
    ...
    getPrefsWidget(pages) {
        const prefsWidget = new Gi.Gtk.Box({
            orientation: Gi.Gtk.Orientation.VERTICAL,
        });
        const stack = new Gi.Gtk.Stack({
            hexpand: true,
        });
        const stackSwitcher = new Gi.Gtk.StackSwitcher({
            halign: Gi.Gtk.Align.CENTER,
            hexpand: true,
        });
        ...
        stack.set_transition_type(Gi.Gtk.StackTransitionType.SLIDE_LEFT_RIGHT);

        const pageProperties = {
            hscrollbar_policy: Gi.Gtk.PolicyType.NEVER,
            vscrollbar_policy: Gi.Gtk.PolicyType.AUTOMATIC,
            vexpand: true,
            hexpand: true,
            visible: true,
        };
        ...
        for (let page of pages) {
            ...
            pagesBtns.push(
                [new Gi.Gtk.Label({ label: title }), _newImageFromIconName(iconName, Gi.Gtk.IconSize.BUTTON)]
            );
        }

        let stBtn = stackSwitcher.get_first_child ? stackSwitcher.get_first_child() : null;
        for (let i = 0; i < pagesBtns.length; i++) {
            const box = new Gi.Gtk.Box({ orientation: Gi.Gtk.Orientation.VERTICAL, spacing: 6, visible: true });
    ...
    _getLegacyPage(optionList, pageProperties) {
        const page = new Gi.Gtk.ScrolledWindow(pageProperties);
        const mainBox = new Gi.Gtk.Box({
            orientation: Gi.Gtk.Orientation.VERTICAL,
            spacing: 5,
            homogeneous: false,
            margin_start: 30,
        ...
        for (let item of optionList) {
            ...
            if (!widget) {
                ...
                frame = new Gi.Gtk.Frame({
                    margin_bottom: 16,
                });

                frameBox = new Gi.Gtk.ListBox({
                    selection_mode: null,
                });
