...
let ThumbnailState;
let ControlsState;
let _originalMaxThumbnailScale;
...
var WorkspaceThumbnailModule = class {
    constructor(gi, ui, misc, me) {
        ...
        ThumbnailState = Ui.WorkspaceThumbnail.ThumbnailState;
    ...
    cleanGlobals() {
        ...
        _originalMaxThumbnailScale = null;
    }
    ...
    _activateModule() {
        ...
        if (!_originalMaxThumbnailScale)
            _originalMaxThumbnailScale = Ui.WorkspaceThumbnail.MAX_THUMBNAIL_SCALE;

        // don't limit max thumbnail scale for other clients than overview, specifically AATWS.
        Ui.WorkspaceThumbnail.MAX_THUMBNAIL_SCALE = 1;
        // Ui.WorkspaceThumbnail.ThumbnailsBox._MAX_THUMBNAIL_SCALE = 1;

        this._overrides.addOverride('WorkspaceThumbnail', Ui.WorkspaceThumbnail.WorkspaceThumbnail.prototype, WorkspaceThumbnailCommon);
        this._overrides.addOverride('ThumbnailsBoxCommon', Ui.WorkspaceThumbnail.ThumbnailsBox.prototype, ThumbnailsBoxCommon);
        ...
        Ui.Main.overview._overview.controls._thumbnailsBox._boxOrientation = opt.ORIENTATION;
    }
    ...
    _disableModule() {
        ...
        Ui.WorkspaceThumbnail.MAX_THUMBNAIL_SCALE = _originalMaxThumbnailScale;
...
const WorkspaceThumbnailCommon = {
    ...
    after__init() {
        ...
        this.layout_manager = new Gi.Clutter.BinLayout();
        ...
        if (opt.SHOW_WST_LABELS) { // 0 - disable
            const getLabel = function () {
                ...
                if (opt.SHOW_WST_LABELS === 2) { // 2 - index + workspace name
                    const settings = Misc.BEFORE_REDRAWExtensionUtils.getSettings('org.gnome.desktop.wm.preferences');
            ...
            this._wsLabel = new Gi.St.Label({
                text: label,
                style_class: 'ws-tmb-label',
                x_align: Gi.Clutter.ActorAlign.FILL,
            ...
            this._wsIndexConId = this.metaWorkspace.connect('notify::workspace-index', () => {
                ...
                if (this._updateLabelTimeout) {
                    Gi.GLib.source_remove(this._updateLabelTimeout);
            ...
            this._nWindowsConId = this.metaWorkspace.connect('notify::n-windows', () => {
                if (this._updateLabelTimeout)
                    return;
                // wait for new data
                this._updateLabelTimeout = Gi.GLib.timeout_add(Gi.GLib.PRIORITY_DEFAULT, 250, () => {
                    const newLabel = getLabel();
                    this._wsLabel.text = newLabel;
                    this._updateLabelTimeout = 0;
        ...
        if (opt.CLOSE_WS_BUTTON_MODE) {
            const closeButton = new Gi.St.Icon({
            ...
            closeButton.connect('enter-event', () => {
                ...
                if (!Gi.Meta.prefs_get_dynamic_workspaces() || (Gi.Meta.prefs_get_dynamic_workspaces() && global.workspace_manager.get_n_workspaces() - 1 !== this.metaWorkspace.index())) {
        ...
        this.connect('enter-event', () => {
            if (opt.CLOSE_WS_BUTTON_MODE && (!Gi.Meta.prefs_get_dynamic_workspaces() || (Gi.Meta.prefs_get_dynamic_workspaces() && global.workspace_manager.get_n_workspaces() - 1 !== this.metaWorkspace.index())))
        ...
        if (opt.SHOW_WS_TMB_BG) {
            this._bgManager = new Ui.Background.BackgroundManager({
    ...
    _closeWorkspace() {
        ...
        if (opt.CLOSE_WS_BUTTON_MODE === 2) {
            const doubleClickTime = Gi.Clutter.Settings.get_default().double_click_time;
    ...
    activate(time) {
        ...
        const stateAdjustment = Ui.Main.overview._overview.controls._stateAdjustment;

        if (stateAdjustment.value === ControlsState.APP_GRID) {
            if (this.metaWorkspace.active) {
                Ui.Main.overview._overview.controls._shiftState(Gi.Meta.MotionDirection.DOWN);
                // if searchActive, hide it immediately
                Ui.Main.overview.searchEntry.set_text('');
            } else {
            ...
            if (this.metaWorkspace.active) {
                Ui.Main.overview._overview.controls._searchController._setSearchActive(false);
    ...
    handleDragOverInternal(source, actor, time) {
        ...
        if (source instanceof Ui.AppDisplay.FolderIcon)
            return Ui.DND.DragMotionResult.COPY_DROP;
    ...
    acceptDropInternal(source, actor, time) {
        ...
        if (source.metaWindow) {
            ...
            Ui.Main.moveWindowToMonitorAndWorkspace(metaWindow,
            ...
            return true;
        } else if (source instanceof Ui.AppDisplay.FolderIcon) {
            if (Misc.shellVersion >= 44) {
                for (let app of source.view._apps) {
                    // const app = Gi.Shell.AppSystem.get_default().lookup_app(id);
...
const ThumbnailsBoxCommon = {
    ...
    acceptDrop(source, actor, x, y, time) {
        if (this._dropWorkspace !== -1) {
            ...
            if (!source.metaWindow &&
                ...
                !(source instanceof Ui.AppDisplay.FolderIcon))
                return false;
            ...
            Ui.Main.wm.insertWorkspace(newWorkspaceIndex);

            if (isWindow) {
                // Move the window to our monitor first if necessary.
                let thumbMonitor = this._thumbnails[newWorkspaceIndex].monitorIndex;
                Ui.Main.moveWindowToMonitorAndWorkspace(source.metaWindow,
            ...
            if (source.app || (!source.app && source.shellWorkspaceLaunch)) {
                ...
                Ui.Main.wm.keepWorkspaceAlive(workspaceManager.get_workspace_by_index(newWorkspaceIndex),
                    Ui.WorkspaceThumbnail.WORKSPACE_KEEP_ALIVE_TIME);
            }
    ...
    handleDragOver(source, actor, x, y, time) {
        ...
        if (!source.metaWindow &&
            (!source.app || !source.app.can_open_new_window()) &&
            (source.app || !source.shellWorkspaceLaunch) &&
            source !== Ui.Main.xdndHandler && !(source instanceof Ui.AppDisplay.FolderIcon))
            return Ui.DND.DragMotionResult.CONTINUE;

        const rtl = Gi.Clutter.get_default_text_direction() === Gi.Clutter.TextDirection.RTL;
        let canCreateWorkspaces = Gi.Meta.prefs_get_dynamic_workspaces();
        ...
        for (let i = 0; i < length; i++) {
            ...
            if (canCreateWorkspaces && source !== Ui.Main.xdndHandler) {
        ...
        if (this._dropWorkspace !== -1)
            ...
            return source.metaWindow ? Ui.DND.DragMotionResult.MOVE_DROP : Ui.DND.DragMotionResult.COPY_DROP;
        else
            return Ui.DND.DragMotionResult.CONTINUE;
    },
...
const ThumbnailsBoxVertical = {
    ...
    vfunc_allocate(box) {
        ...
        let rtl = Gi.Clutter.get_default_text_direction() === Gi.Clutter.TextDirection.RTL;
        ...
        let indicatorTopFullBorder = indicatorThemeNode.get_padding(Gi.St.Side.TOP) + indicatorThemeNode.get_border_width(Gi.St.Side.TOP);
        let indicatorBottomFullBorder = indicatorThemeNode.get_padding(Gi.St.Side.BOTTOM) + indicatorThemeNode.get_border_width(Gi.St.Side.BOTTOM);
        let indicatorLeftFullBorder = indicatorThemeNode.get_padding(Gi.St.Side.LEFT) + indicatorThemeNode.get_border_width(Gi.St.Side.LEFT);
        let indicatorRightFullBorder = indicatorThemeNode.get_padding(Gi.St.Side.RIGHT) + indicatorThemeNode.get_border_width(Gi.St.Side.RIGHT);
        ...
        if (this._dropPlaceholderPos === -1) {
            ...
            if (Gi.Meta.later_add)
                Gi.Meta.later_add(Gi.Meta.LaterType.BEFORE_REDRAW, laterFunc);
            else
                global.compositor.get_laters().add(Gi.Meta.LaterType.BEFORE_REDRAW, laterFunc);
        }

        let childBox = new Gi.Clutter.ActorBox();
