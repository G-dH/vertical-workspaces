...
export var LayoutModule = class {
    ...
    _activateModule() {
        ...
        if (!this._originalUpdateHotCorners)
            this._originalUpdateHotCorners = Ui.Layout.LayoutManager.prototype._updateHotCorners;

        this._overrides = new Me.Util.Overrides();
        this._overrides.addOverride('LayoutManager', Ui.Layout.LayoutManager.prototype, LayoutManagerCommon);
        this._overrides.addOverride('HotCorner', Ui.Layout.HotCorner.prototype, HotCornerCommon);

        Ui.Main.layoutManager._updateHotCorners = LayoutManagerCommon._updateHotCorners.bind(Ui.Main.layoutManager);

        Ui.Main.layoutManager._updatePanelBarrier();
    ...
    _disableModule() {
        ...
        Ui.Main.layoutManager._updateHotCorners = this._originalUpdateHotCorners;
        Ui.Main.layoutManager._updateHotCorners();
        this._clearGlobals();
    }
...
const LayoutManagerCommon = {
    ...
    _updateHotCorners() {
        ...
        for (let i = 0; i < this.monitors.length; i++) {
            ...
            if (haveCorner) {
                let corner = new Ui.Layout.HotCorner(this, monitor, cornerX, cornerY);
...
const HotCornerCommon = {
        ...
        if (size > 0) {
            ...
            if (opt.HOT_CORNER_POSITION <= 1) {
                ...
                this._horizontalBarrier = new Gi.Meta.Barrier({
                    display: global.display,
                    x1: this._x, x2: this._x + (extendH ? monitor.width : size), y1: this._y, y2: this._y,
                    directions: Gi.Meta.BarrierDirection.NEGATIVE_Y,
                });
    ...
    _toggleOverview() {
        if (!opt.HOT_CORNER_ACTION || (!opt.HOT_CORNER_FULLSCREEN && this._monitor.inFullscreen && !Ui.Main.overview.visible))
            return;

        if (Ui.Main.overview.shouldToggleByCornerOrButton()) {
            if ((opt.HOT_CORNER_ACTION === 1 && !Me.Util.isCtrlPressed()) || (opt.HOT_CORNER_ACTION === 2 && Me.Util.isCtrlPressed()))
                this._toggleWindowPicker(true);
            else if ((opt.HOT_CORNER_ACTION === 2 && !Me.Util.isCtrlPressed()) || (opt.HOT_CORNER_ACTION === 1 && Me.Util.isCtrlPressed()) || (opt.HOT_CORNER_ACTION === 3 && Me.Util.isCtrlPressed()))
                this._toggleApplications(true);
            else if (opt.HOT_CORNER_ACTION === 3 && !Me.Util.isCtrlPressed())
                this._toggleWindowSearchProvider();
            if (opt.HOT_CORNER_RIPPLES && Ui.Main.overview.animationInProgress)
                this._ripples.playAnimation(this._x, this._y);
        }
    },

    _toggleWindowPicker(leaveOverview = false) {
        if (Ui.Main.overview._shown && (leaveOverview || !Ui.Main.overview.dash.showAppsButton.checked)) {
            Ui.Main.overview.hide();
        } else if (Ui.Main.overview.dash.showAppsButton.checked) {
            ...
            if (!Gi.Meta.is_wayland_compositor() && focusWindow && focusWindow.wm_class.includes('VirtualBox Machine')) {
                // following should help when windowed VBox Machine has focus.
                global.stage.set_key_focus(Ui.Main.panel);
                // key focus doesn't take the effect immediately, we must wait for it
                // still looking for better solution!
                _timeouts.releaseKeyboardTimeoutId = Gi.GLib.timeout_add(
    ...
    _toggleApplications(leaveOverview = false) {
        if ((leaveOverview && Ui.Main.overview._shown) || Ui.Main.overview.dash.showAppsButton.checked) {
            Ui.Main.overview.hide();
        } else {
            const focusWindow = global.display.get_focus_window();
            // at least GS 42 is unable to show overview in X11 session if VirtualBox Machine window grabbed keyboard
            ...
            } else if (Ui.Main.overview._shown) {
                Ui.Main.overview.dash.showAppsButton.checked = true;
            } else {
                Ui.Main.overview.show(2); // 2 for App Grid
            }
        }
    },

    _toggleWindowSearchProvider() {
        if (!Ui.Main.overview._overview._controls._searchController._searchActive) {
            ...
            // Ui.Main.overview.searchEntry.text = '';
            Ui.Main.overview.hide();
        }
    },
};
