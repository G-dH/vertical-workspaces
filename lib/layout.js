...
const LayoutManagerCommon = {
    ...
    _updateHotCorners() {
        ...
        // but avoid parallel calls from the default callback (todo - disconnect the callback)
        if (this._removingCorners) {
            this._removingCorners = true;
            this.hotCorners.forEach(corner => corner?.destroy());
            this.hotCorners = [];
            this._removingCorners = false;
        }
        ...
        // fixed size in case the panel is hidden
        let size = this.panelBox.visible ? this.panelBox.height : 32;
...
const HotCornerCommon = {
    ...
    setBarrierSize(size, notMyCall = true) {
        ...
        // fix calls from the original _updateHotCorners() callback
        if (notMyCall) {
            Main.layoutManager._updateHotCorners();
            return;
        }
