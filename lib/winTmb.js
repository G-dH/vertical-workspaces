...
var WinTmbModule = class {
    ...
    update(reset) {
        ...
        this._moduleEnabled = true;// opt.get('winTmbModule');
    ...
    createThumbnail(metaWin) {
        this.windowThumbnails.push(new WindowThumbnail(metaWin, this, {
            'height': Math.floor(0.15 * global.display.get_monitor_geometry(global.display.get_current_monitor()).height),
            'thumbnailsOnScreen': this.windowThumbnails.length,
        }));
        if (!this._stateAdjustmentConId)
            this._stateAdjustmentConId = Ui.Main.overview._overview.controls._stateAdjustment.connectObject('notify::value', () => this.hideThumbnails(), this);
    }

    hideThumbnails() {
        if (!this._thumbnailsHidden && (!opt.OVERVIEW_MODE2 || opt.WORKSPACE_MODE)) {
            this.windowThumbnails.forEach(tmb => {
                tmb.ease({
                    opacity: 0,
                    duration: 200,
    ...
    removeAllThumbnails() {
        this.windowThumbnails.forEach(tmb => tmb.remove());
    }
};
