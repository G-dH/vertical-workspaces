...
export const Options = class Options {
    constructor(me) {
        ...
        this.options = {
            ...
            profileData1: ['a{ss}', 'profile-data-1'],
            profileData2: ['a{ss}', 'profile-data-2'],
            profileData3: ['a{ss}', 'profile-data-3'],
            profileData4: ['a{ss}', 'profile-data-4'],
            profileData5: ['a{ss}', 'profile-data-5'],
        };
        this.cachedOptions = {};
        this._updateSettings();
    ...
    valueChanged(key) {
        // Compare cached value with the one stored in gsettings
        for (const name of Object.keys(this.options)) {
            if (this.options[name][1] === key) {
                return !this.cachedOptions[name][1].equal(
                    this._gsettings.get_value(key)
                );
            }
        }
        return false;
    ...
    get(option, updateCache = false) {
        ...
        if (updateCache || this.cachedOptions[option] === undefined) {
            ...
            // Store unpacked value for efficiency and original value for change detection
            const value = gSettings.get_value(key);
            this.cachedOptions[option] = [value.deep_unpack(), value];
        }

        return this.cachedOptions[option][0];
    }
    ...
    _updateSettings() {
        ...
        this.SEARCH_DELAY = this.get('searchDelay');
