...
export var Options = class Options {
    constructor(gi, me) {
        Gi = gi;
        Me = me;
        ...
        this.connect('changed', () => {
            if (this._writeTimeoutId)
                Gi.GLib.Source.remove(this._writeTimeoutId);

            this._writeTimeoutId = Gi.GLib.timeout_add(
    ...
    destroy() {
        ...
        if (this._writeTimeoutId) {
            Gi.GLib.source_remove(this._writeTimeoutId);
            this._writeTimeoutId = 0;
        }
    }
    ...
    storeProfile(index) {
        ...
        this._gsettings.set_value(`profile-data-${index}`, new Gi.GLib.Variant('a{ss}', profile));
    }
