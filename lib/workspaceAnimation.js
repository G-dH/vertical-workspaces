...
export var WorkspaceAnimationModule = class {
    ...
    update(reset) {
        /*if (!Ui.WorkspaceAnimation.MonitorGroup)
            return;*/
    ...
    _activateModule() {
        ...
        if (opt.STATIC_WS_SWITCHER_BG) {
            ...
            //this._overrides.addOverride('WorkspaceAnimationMonitorGroup', Ui.WorkspaceAnimation.MonitorGroup.prototype, MonitorGroup);
            this._overrides.addOverride('WorkspaceAnimationController', Ui.WorkspaceAnimation.WorkspaceAnimationController.prototype, WorkspaceAnimationController);
        }
    ...
    _connectWsAnimationSwipeTracker(reset = false) {
        if (reset) {
            if (this._wsAnimationSwipeBeginId) {
                Ui.Main.wm._workspaceAnimation._swipeTracker.disconnect(this._wsAnimationSwipeBeginId);
                this._wsAnimationSwipeBeginId = 0;
            }
            if (this._wsAnimationSwipeEndId) {
                Ui.Main.wm._workspaceAnimation._swipeTracker.disconnect(this._wsAnimationSwipeEndId);
                this._wsAnimationSwipeEndId = 0;
            }
        } else if (!this._wsAnimationSwipeBeginId) {
            // display ws switcher popup when gesture begins and connect progress
            this._wsAnimationSwipeBeginId = Ui.Main.wm._workspaceAnimation._swipeTracker.connect('begin', () => this._connectWsAnimationProgress(true));
            // we want to be sure that popup with the final ws index show up when gesture ends
            this._wsAnimationSwipeEndId = Ui.Main.wm._workspaceAnimation._swipeTracker.connect('end', (tracker, duration, endProgress) => this._connectWsAnimationProgress(false, endProgress));
        }
    }

    _connectWsAnimationProgress(connect, endProgress = null) {
        if (Ui.Main.overview.visible)
            return;

        if (connect && !this._wsAnimationSwipeUpdateId) {
            this._wsAnimationSwipeUpdateId = Ui.Main.wm._workspaceAnimation._swipeTracker.connect('update', (tracker, progress) => this._showWsSwitcherPopup(progress));
        } else if (!connect && this._wsAnimationSwipeUpdateId) {
            Ui.Main.wm._workspaceAnimation._swipeTracker.disconnect(this._wsAnimationSwipeUpdateId);
            this._wsAnimationSwipeUpdateId = 0;
    ...
    _showWsSwitcherPopup(progress) {
        ...
        if (Ui.Main.wm._workspaceSwitcherPopup === null) {
            Ui.Main.wm._workspaceSwitcherPopup = new Ui.WorkspaceSwitcherPopup.WorkspaceSwitcherPopup();
            Ui.Main.wm._workspaceSwitcherPopup.connect('destroy', () => {
                Ui.Main.wm._workspaceSwitcherPopup = null;
            });
        }

        Ui.Main.wm._workspaceSwitcherPopup.display(wsIndex);
    }
};
...
const WorkspaceAnimationController = {
    _prepareWorkspaceSwitch(workspaceIndices) {
        ...
        const monitors = Gi.Meta.prefs_get_workspaces_only_on_primary()
            ? [Ui.Main.layoutManager.primaryMonitor] : Ui.Main.layoutManager.monitors;

        for (const monitor of monitors) {
            if (Gi.Meta.prefs_get_workspaces_only_on_primary() &&
                monitor.index !== Ui.Main.layoutManager.primaryIndex)
                continue;

            const group = new MonitorGroup(monitor, workspaceIndices, this.movingWindow);

            Ui.Main.uiGroup.insert_child_above(group, global.window_group);

            switchData.monitors.push(group);
        }

        Gi.Meta.disable_unredirect_for_display(global.display);
    }
}
...
const WorkspaceGroup = GObject.registerClass(
    class WorkspaceGroup extends Clutter.Actor {
        _init(workspace, monitor, movingWindow) {
            ...
            if (this._workspace) {
                this._background = new Gi.Meta.BackgroundGroup();
...
const MonitorGroup = GObject.registerClass({
    ...
    _init(monitor, workspaceIndices, movingWindow) {
        ...
        const constraint = new Ui.Layout.MonitorConstraint({index: monitor.index});
        ...
        for (const i of workspaceIndices) {
            ...
            if (i > 0 && vertical && !fullscreen && monitor.index === Ui.Main.layoutManager.primaryIndex) {
                // We have to shift windows up or down by the height of the panel to prevent having a
                // visible gap between the windows while switching workspaces. Since fullscreen windows
                // hide the panel, they don't need to be shifted up or down.
                y -= Ui.Main.panel.height;
            }
