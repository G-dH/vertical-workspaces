    ...
    _init(monitor, workspaceIndices, movingWindow) {
        ...
        if (monitor.index === Main.layoutManager.primaryIndex) {
            ...
            this.connect('destroy', () => {
                // Workaround for an upstream bug that occurs when the overview animation
                // is triggered during the workspace switching animation.
                // In this case, onComplete() in _switchWorkspaceEnd() is not executed,
                // causing workspaceManager and overview to get out of sync.
                ...
                if (workspacesAdj.value !== workspaceManager.get_active_workspace().index())
                    workspaceManager.get_workspace_by_index(workspacesAdj.value).activate(global.get_current_time());*/
        ...
        const ignoredWmClasses = [
            'conky',
            'ding',
            'Gjs', // the original DING extension window class string
        ];
        function shouldBeIgnored(metaWin) {
            return metaWin.is_on_all_workspaces() &&
                ignoredWmClasses.filter(string =>
                    metaWin.wm_class?.includes(string)
                ).length > 0;
        }
        global.get_window_actors().forEach(actor => {
            const metaWin = actor.metaWindow;
            if (metaWin.get_monitor() === this._monitor.index &&
            !shouldBeIgnored(metaWin)) {
        ...
        // to-do - actors removed during transition need to be removed from the list to avoid access to destroyed actor
        this.connect('destroy', () => {
