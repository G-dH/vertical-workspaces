...
let ModifierType; // = imports.gi.Clutter.ModifierType;
...
export var WindowSearchProviderModule = class {
    constructor(gi, ui, misc, me) {
        ...
        _  = Me.gettext;
        ModifierType = Gi.Clutter.ModifierType;
    ...
    update(reset) {
        this._moduleEnabled = opt.get('windowSearchProviderModule');
...
const WindowSearchProvider = class WindowSearchProvider {
    constructor() {
        ...
        this.appInfo = Gi.Gio.AppInfo.create_from_commandline('true', _('Open Windows'), null);
        this.appInfo.get_description = () => _('Search open windows');
        this.appInfo.get_name = () => _('Open Windows');
        this.appInfo.get_id = () => this.id;
        this.appInfo.get_icon = () => Gi.Gio.icon_new_for_string('focus-windows-symbolic');
    ...
    getResultMeta(resultId) {
        ...
        const app = Gi.Shell.WindowTracker.get_default().get_window_app(result.window);
        return {
            'id': resultId,
            ...
            'createIcon': size => {
                return app
                    ...
                    : new Gi.St.Icon({ icon_name: 'icon-missing', icon_size: size });
            },
        };
    }

    makeResult(window, i) {
        const app = Gi.Shell.WindowTracker.get_default().get_window_app(window);
    ...
    activateResult(resultId/* , terms, timeStamp*/) {
        ...
        if (!this.action) {
            const result = this.windows[resultId];
            Ui.Main.activateWindow(result.window);
            return;
        }
    ...
    _closeWindows(ids) {
        ...
        Ui.Main.notify('Window Search Provider', `Closed ${ids.length} windows.`);
    }
    ...
    getInitialResultSet(terms, callback) {
        ...
        global.display.get_tab_list(Gi.Meta.TabList.NORMAL, null).filter(w => w.get_workspace() !== null).map(
            (v, i) => {
