...
var OverviewControlsModule = class {
    ...
    _addClickToCloseOverview(reset) {
        ...
        if (!reset && !this._overviewClickAction) {
            this._overviewClickAction = new Clutter.ClickAction();
            this._overviewClickAction.connect('clicked', () => {
                // if (controls._stateAdjustment.value < ControlsState.APP_GRID)
                Main.overview.hide();
            });
            controls.reactive = true;
            controls.add_action(this._overviewClickAction);

            // Prevent accidentally closing the overview by clicking between search results
            this._searchResultsClickBlocker = new Clutter.ClickAction();
            this._searchResultsClickBlocker.connect('clicked', () => Clutter.EVENT_STOP);

            searchResults.reactive = true;
            searchResults.add_action(this._searchResultsClickBlocker);
        } else if (this._overviewClickAction) {
            ...
            searchResults.remove_action(this._searchResultsClickBlocker);
            this._searchResultsClickBlocker = null;
        }
    }
};
