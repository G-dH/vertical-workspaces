...
const ControlsManagerCommon = {
    prepareToEnterOverview() {
        ...
        // Setting the background actors' z_position to 1 seems to fix
        // the glitching issue that occurs when changing the blur effect radius
        // while multiple monitors are connected.
        // In this case, we need to manually keep the background actors
        // at the bottom of the overviewGroup stack.
        if (this._bgManagers) {
            for (const bgManager of this._bgManagers)
                Main.layoutManager.overviewGroup.set_child_at_index(bgManager.backgroundActor, 0);
        }
    ...
    _setBackground(reset = false) {
        ...
        for (const monitor of Main.layoutManager.monitors) {
            ...
            bgManager.backgroundActor.z_position = 1;
