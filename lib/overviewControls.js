...
const ControlsManagerCommon = {
    ...
    _updateThumbnailsBox(animate) {
        ...
        const show = shouldShow &&
            !(!opt.SHOW_WS_TMB_WIN_PICKER && !opt.SHOW_WS_TMB_APPGRID/* && !opt.SHOW_WS_TMB_SEARCH*/) &&
            !(!opt.MAX_THUMBNAIL_SCALE && currentState <= 1 && !searchActive) &&
            !(!opt.SHOW_WS_TMB_APPGRID && currentState === 2 && !(searchActive && !opt.SEARCH_APP_GRID_MODE)) &&
            !(!opt.SHOW_WS_TMB_SEARCH && searchActive && !(opt.SEARCH_APP_GRID_MODE && this.dash.showAppsButton.checked)) &&
            !(!opt.MAX_THUMBNAIL_SCALE && currentState > 1 && opt.WS_ANIMATION_ALL);
    ...
    _onSearchChanged() {
        ...
        this._updateSearchResultsBlurEffectIfNeeded();
        this._fadeSearchResults(searchActive);
    },

    _updateSearchResultsBlurEffectIfNeeded() {
        if (((opt.OVERVIEW_MODE2 && !opt.WORKSPACE_MODE) || opt.SEARCH_RESULTS_BG_STYLE) && this._searchController.searchActive)
            this._addBlurEffectToSearchResults();
        else
            this._removeBlurEffectFromSearchResults();
    },
    ...
    _fadeSearchResults(searchActive) {
        ...
        this._searchController._searchResults.ease({
            ...
            mode: Clutter.AnimationMode.EASE_OUT_QUAD,
            onStopped: () => {
                this._searchController.visible = this._searchController.searchActive;
    ...
    _fadeAppDisplay(searchActive) {
        if (this.dash.showAppsButton.checked) {
            ...
            this._appDisplay.ease({
                ...
                duration: searchActive ? opt.SEARCH_DELAY : 150,
                mode: Clutter.AnimationMode.EASE_OUT_QUAD,
                onStopped: () => this._updateAppDisplayVisibility(),
            });
        }
    },
    ...
    _updateSearchStyle(reset) {
        const styles = [
            'non',
            'search-results-bg-dark',
            'search-results-bg-dark-glass',
            'search-results-bg-light-glass',
        ];
        const style = opt.OVERVIEW_MODE2 && !opt.WORKSPACE_MODE && !opt.SEARCH_RESULTS_BG_STYLE
            ? 2
            : opt.SEARCH_RESULTS_BG_STYLE;
        styles.forEach(style => this._searchController._searchResults.remove_style_class_name(style));
        if (!reset && !(this.dash.showAppsButton.checked && opt.SEARCH_APP_GRID_MODE) && (
            ...
            this._searchController._searchResults.add_style_class_name(styles[style]);
            this._searchEntry.add_style_class_name('search-entry-om2');
        } else {
            this._searchController._searchResults.remove_style_class_name(styles[style]);
            this._searchEntry.remove_style_class_name('search-entry-om2');
        }
    },

    _addBlurEffectToSearchResults() {
        if (!this._searchResultsBlurEffect)
            this._searchResultsBlurEffect = new Shell.BlurEffect({ brightness: 1, mode: Shell.BlurMode.BACKGROUND });

        if (!this._searchController._searchResults.get_effects().length)
            this._searchController._searchResults.add_effect_with_name('blur', this._searchResultsBlurEffect);
        const radius = this._getRadiusProperty(this._searchResultsBlurEffect);
        this._searchResultsBlurEffect[radius] = opt.RESULTS_BLUR_RADIUS;
        if (!this._searchResultsRepainter) {
            const searchResults = this._searchController._searchResults;
            this._searchResultsRepainter = Me.Util.forwardRepaint(searchResults._content, searchResults);
        }
    },

    _removeBlurEffectFromSearchResults() {
        if (this._searchResultsRepainter) {
            this._searchResultsRepainter.removeAll();
            this._searchResultsRepainter = null;
        }
        this._searchController._searchResults.remove_effect_by_name('blur');
    },

    realizeAppDisplayAndFinishStartup() {
    ...
    _idleOverviewAnimation(state, callback) {
        ...
        const timeout = opt.timeout ?? 200;
        Me.run.timeouts.overviewIdleLimiter = GLib.timeout_add(
            GLib.PRIORITY_DEFAULT,
            timeout,
            () => {
                // print('>    overviewIdleLimiter triggered');
                if (Me.run.timeouts.overviewIdle) {
                    GLib.source_remove(Me.run.timeouts.overviewIdle);
                    Me.run.timeouts.overviewIdle = 0;
                    this._startOverviewAnimation(state, callback);
...
const ControlsManagerLayoutCommon = {
    ...
    _shouldAnimateWorkspace(state) {
        if (!opt.WS_ANIMATION)
            return false;

        const { searchActive } = this._searchController;
        const searchViewActive = searchActive && !(this._dash.showAppsButton.checked && opt.SEARCH_APP_GRID_MODE);
        const animateSearch = opt.SHOW_WS_TMB_SEARCH && searchViewActive;
        const animateAppGrid  = !searchViewActive && opt.SHOW_WS_TMB_APPGRID && state === ControlsState.APP_GRID;

        return animateSearch || animateAppGrid;
    },
    ...
    _computeWorkspacesBoxForState(state, box, leftBoxOffset, rightBoxOffset, topBoxOffset, bottomBoxOffset, centeredBoxOffset, dashWidth, dashHeight, searchEntryHeight, searchEntryPositionTop) {
        ...
        case ControlsState.WINDOW_PICKER:
        case ControlsState.APP_GRID:
            if (this._workspacesThumbnails.visible && this._shouldAnimateWorkspace(state) && state === ControlsState.APP_GRID) {
                if (opt.ORIENTATION)
                ...
                if (opt.ORIENTATION) {
                    const wsTmbBox = this._getThumbnailsBoxForState(-1, dashWidth, dashHeight);
                    const tmbBoxWidth = wsTmbBox.get_width();
                    if (!this._workspacesThumbnails.visible) {
                        leftBoxOffset += opt.WS_TMB_LEFT ? tmbBoxWidth : 0;
                        rightBoxOffset += opt.WS_TMB_RIGHT ? tmbBoxWidth : 0;
                    }
                } else {
                    const wsTmbBox = this._getThumbnailsBoxForState(-1, dashWidth, dashHeight, searchEntryHeight, searchEntryPositionTop);
                    const tmbBoxHeight = wsTmbBox.get_height();
                    topBoxOffset += opt.WS_TMB_TOP ? tmbBoxHeight : 0;
                    bottomBoxOffset += opt.WS_TMB_BOTTOM ? tmbBoxHeight : 0;
                }
...
const ControlsManagerLayoutVertical = {
    _getThumbnailsBoxForState(state, dashWidth, dashHeight) {
        ...
        let { searchActive } = this._searchController;

        // Size of the workspace shouldn't be affected by the thumbnails size change
        const computeForWorkspaceBox = state === -1;
        if (computeForWorkspaceBox) {
            searchActive = false;
        ...
        if (searchActive && !(opt.SEARCH_APP_GRID_MODE && this._dash.showAppsButton.checked))
            maxWsTmbScale = opt.MAX_THUMBNAIL_SCALE_SEARCH;
        else if (state === ControlsState.APP_GRID)
            maxWsTmbScale = opt.MAX_THUMBNAIL_SCALE_APPGRID;

        // If maxWsTmbScale is 0, get_preferred_width() returns maximum size instead of 0
        if (maxWsTmbScale) {
    ...
    vfunc_allocate(container, box) {
        ...
        if (opt.SHOW_WS_TMB) {
            const getState = state =>
                state <= ControlsState.WINDOW_PICKER ||
                // Show the thumbnails before workspacesDisplay is hidden to avoid glitches
...
const ControlsManagerLayoutHorizontal = {
    ...
    vfunc_allocate(container, box) {
        ...
        const leftBoxOffset = opt.DASH_LEFT ? dashWidth : spacing;
        const rightBoxOffset = opt.DASH_RIGHT ? dashWidth : spacing;
        let topBoxOffsetWs = opt.DASH_TOP ? dashHeight : halfSpacing;
        let topBoxOffset = topBoxOffsetWs + (opt.WS_TMB_TOP ? wsTmbHeight + spacing : 0);
        const bottomBoxOffsetWs = opt.DASH_BOTTOM ? dashHeight : spacing;
        const bottomBoxOffset = bottomBoxOffsetWs + (opt.WS_TMB_BOTTOM ? wsTmbHeight + spacing : 0);
        const centeredBoxOffset = Math.max(leftBoxOffset, rightBoxOffset);
        ...
        // Add searchEntry height if needed
        const searchEntryOffset = opt.SHOW_SEARCH_ENTRY ? searchEntryHeight + spacing : 0;
        topBoxOffset += searchEntryOffset;
        topBoxOffsetWs += searchEntryOffset;
