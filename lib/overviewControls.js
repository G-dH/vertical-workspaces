...
const ControlsManagerLayoutVertical = {
    vfunc_allocate(container, box) {
        ...
        const opaqueSearchResults = (opt.OVERVIEW_MODE2 && !opt.WORKSPACE_MODE && !this._dash.showAppsButton.checked) || (opt.SEARCH_RESULTS_BG_STYLE && !this._dash.showAppsButton.checked);
...
const ControlsManagerLayoutHorizontal = {
    vfunc_allocate(container, box) {
        ...
        if (!opt.SHOW_SEARCH_ENTRY && opt.SEARCH_APP_GRID_MODE)
            topBoxOffsetAppGrid += Math.floor(searchEntryHeight / 2);
        params = [
            box,
