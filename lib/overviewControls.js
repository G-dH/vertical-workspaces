...
const ControlsManagerCommon = {
    prepareToEnterOverview() {
        ...
        if (!Main.layoutManager._startingUp && !Main.overview._overview.controls._stateAdjustment.value >= 1) {
            ...
            // COPY OF OVERLAY KEY PRIMARY ACTION SWITCH FOR GESTURE OPEN
            if (!Main.overview._shown) {
                switch (opt.OVERLAY_KEY_PRIMARY) {
                case 0: // Disabled
                    // Do nothing (gesture will proceed with existing settings)
                    break;
                ...
                case 2: // Default overview
                    ...
                    break;
                case 3: // App grid
                    // Directly show app grid; return to avoid continuing with window picker assumptions
    ...
    _updateSearchEntry(currentState, finalState, opacity) {
        ...
        if (currentState > ControlsState.WINDOW_PICKER) {
            ...
            this._searchEntryBin.visible = opt.SHOW_SEARCH_ENTRY || this._searchInProgress;
    ...
    _updateSearchEntryVisibility(searchActive) {
        const entry = this._searchEntryBin;
