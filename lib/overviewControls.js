...
const { Clutter, GLib, GObject, St, Shell, Meta } = imports.gi;
...
function update(reset = false) {
    ...
    // if DtD is enabled, we need to replace _prepareStartupAnimation() to workaround the mess in the overview
    const dashToDockEnabled = _Util.getEnabledExtensions('dash-to-dock').length ||
                              _Util.getEnabledExtensions('ubuntu-dock').length;
    if (dashToDockEnabled)
        _overrides.addOverride('LayoutManagerDtD', Layout.LayoutManager.prototype, LayoutManagerDtD);
}
...
const ControlsManager = {
    ...
    _updateWorkspacesDisplay() {
        ...
        if (!Main.layoutManager._startingUp && ((!opt.SHOW_WS_PREVIEW_BG && !opt.OVERVIEW_MODE2) || animateOverviewMode2)) {
            if (!tmbBox._translationOriginal || Math.abs(tmbBox._translationOriginal[0]) > 500) { // swipe gesture can call this calculation before tmbBox is finalized, giving nonsense width
                const [dashTranslationX, dashTranslationY, tmbTranslationX, tmbTranslationY, searchTranslationY] = _Util.getOverviewTranslations(opt, dash, tmbBox, searchEntryBin);
    ...
    async runStartupAnimation(callback) {
        ...
        const onComplete = function () {
            ...
            const appDisplayModule = Me.imports.lib.appDisplay;
            ...
            if (opt.get('appDisplayModule')) {
                appDisplayModule.exposeAppGrid();
                appDisplayModule._updateIconPositions();
                // realize app grid for smoother first animation
                _timeouts.startup1 = GLib.idle_add(GLib.PRIORITY_LOW, () => {
                    appDisplay._redisplay();
                    appDisplayModule.restoreAppGrid(false);
                    _timeouts.startup1 = 0;
                    return GLib.SOURCE_REMOVE;
                });
            }
            _timeouts.startup2 = GLib.idle_add(
                GLib.PRIORITY_LOW,
                () => {
    ...
    _finishStartupSequence() {
        _timeouts.finishStartup = GLib.idle_add(
            GLib.PRIORITY_LOW, () => {
                ...
                } else if (!opt.STARTUP_STATE && _Util.dashNotDefault()) {
                    Main.overview.show();
                }
    ...
    _setInitialTranslations() {
        const dash = this.dash;
        const tmbBox = this._thumbnailsBox;
        tmbBox.visible = false;
        const searchEntryBin = this._searchEntryBin;
        const [dashTranslationX, dashTranslationY, tmbTranslationX, tmbTranslationY, searchTranslationY] = _Util.getOverviewTranslations(opt, dash, tmbBox, searchEntryBin);
        if (!_Util.dashNotDefault()) {
            dash.translation_x = dashTranslationX;
...
const LayoutManagerDtD = {
    async _prepareStartupAnimation() {
        ...
        if (Meta.is_restart() || _Util.dashIsDashToDock()) {
