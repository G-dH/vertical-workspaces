        ...
        if (!reset && !this._wsChangedConnId) {
            this._wsChangedConnId = global.workspaceManager.connect('active-workspace-changed',
                () => {
                    ...
                    this._removeWinPreviewOverlayDelay();
                    this._winPreviewOverlayDelayId = GLib.timeout_add(GLib.PRIORITY_DEFAULT_IDLE, delay * slowDownFactor, () => {
...
const ControlsManagerCommon = {
    ...
    _idleOverviewAnimation(state, callback) {
        ...
        Me.run.timeouts.overviewIdle = GLib.idle_add(GLib.PRIORITY_DEFAULT_IDLE, () => {
            if (Me.run.timeouts.overviewDelayLimiter) {
                GLib.source_remove(Me.run.timeouts.overviewDelayLimiter);
                Me.run.timeouts.overviewDelayLimiter = 0;
            }
            this._startOverviewAnimation(state, callback);
        ...
        Me.run.timeouts.overviewDelayLimiter = GLib.timeout_add(
            GLib.PRIORITY_DEFAULT,
            timeout,
            () => {
                if (Me.run.timeouts.overviewIdle) {
