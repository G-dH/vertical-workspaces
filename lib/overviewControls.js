...
const ControlsManagerCommon = {
    ...
    _updateOverview() {
        ...
        this._updateOverviewStackOrder(currentState, finalState, progress);
    },
    ...
    _updateTranslationsIfNeeded(tmbBox, dash, searchEntryBin, panelBox) {
        ...
        if (opt.PANEL_OVERVIEW_ONLY && Main.layoutManager.panelBox.get_parent() === Main.layoutManager.overviewGroup)
            panelBox._translationOriginal = (opt.PANEL_POSITION_TOP ? -1 : 1) * panelBox.height;
        else
            delete panelBox._translationOriginal;
    },
    ...
    _updateOverviewStackOrder(currentState, finalState, progress) {
        if (!this.dash._isAbove && progress > 0 && opt.OVERVIEW_MODE2 && !opt.WORKSPACE_MODE) {
            this._setDashAboveSiblings();
        } else if (!this.dash._isAbove && currentState === 1 && finalState > ControlsState.HIDDEN) {
            this._setDashAboveSiblings();
