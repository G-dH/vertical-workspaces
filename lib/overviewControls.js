...
export const OverviewControlsModule = class {
    ...
    _activateModule() {
        ...
        if (opt.ORIENTATION === Gi.Clutter.Orientation.VERTICAL)
            this._overrides.addOverride('ControlsManagerLayout', Ui.Main.overview._overview.controls.layoutManager, ControlsManagerLayoutVertical);
        else
            this._overrides.addOverride('ControlsManagerLayout', Ui.Main.overview._overview.controls.layoutManager, ControlsManagerLayoutHorizontal);
        ...
        if (dashToDockEnabled)
            this._overrides.addOverride('LayoutManagerDtD', Ui.Layout.LayoutManager.prototype, LayoutManagerDtD);*/
    }
...
const ControlsManagerCommon = {
    ...
    _updateWorkspacesDisplay() {
        ...
        // remove ws thumbnails rescale animation during this scale transition, it is redundant and delayed
        if (currentState < 2 && currentState > 1)
            this._thumbnailsBox.RESCALE_ANIMATION_TIME = 0;
        else
            this._thumbnailsBox.RESCALE_ANIMATION_TIME = 200;
        ...
        if (!Ui.Main.layoutManager._startingUp && ((!opt.SHOW_WS_PREVIEW_BG && !opt.OVERVIEW_MODE2) || animateOverviewMode2)) {
            if (!tmbBox._translationOriginal || Math.abs(tmbBox._translationOriginal[0]) > 500) { // swipe gesture can call this calculation before tmbBox is finalized, giving nonsense width
                const [dashTranslationX, dashTranslationY, tmbTranslationX, tmbTranslationY, searchTranslationY] =  this.getOverviewTranslations(opt, dash, tmbBox, searchEntryBin);
    ...
    async runStartupAnimation(callback) {
        ...
        const [dashTranslationX, dashTranslationY, tmbTranslationX, tmbTranslationY, searchTranslationY] =
             this.getOverviewTranslations(opt, dash, tmbBox, searchEntryBin);
        ...
        const onComplete = function () {
            ...
            if (opt.get('appDisplayModule')) {
                ...
                _timeouts.startup1 = Gi.GLib.idle_add(Gi.GLib.PRIORITY_LOW, () => {
                    ...
                    appDisplayModule._restoreAppGrid(false);
    ...
    getOverviewTranslations(opt, dash, tmbBox, searchEntryBin) {
        // const tmbBox = Ui.Main.overview._overview._controls._thumbnailsBox;
        const animationsDisabled = !Gi.St.Settings.get().enable_animations || (opt.SHOW_WS_PREVIEW_BG && !opt.OVERVIEW_MODE2);
        ...
        if (tmbBox.visible) {
            ...
            case 1: // right
                ...
                break;
            case 0: // top
                offset = 10 + (dash?.visible && opt.DASH_TOP ? dash.height : 0) + Ui.Main.panel.height;
                tmbTranslationX = 0;
                tmbTranslationY = -tmbHeight - offset;
                break;
            case 2: // bottom
                offset = 10 + (dash?.visible && opt.DASH_BOTTOM ? dash.height : 0) + Ui.Main.panel.height;  // just for case the panel is at bottom
                tmbTranslationX = 0;
                tmbTranslationY = tmbHeight + offset;
                break;
            }
        ...
        if (dash?.visible) {
            ...
            case 0: // top
                dashTranslationX = 0;
                dashTranslationY = -dashHeight - dash.margin_bottom - Ui.Main.panel.height;
                break;
            case 1: // right
...
const ControlsManagerLayoutVertical = {
    ...
    vfunc_allocate(container, box) {
        ...
        if (opt.SHOW_WS_TMB) {
            ...
            let wsTmbX;
            if (opt.WS_TMB_RIGHT)
                wsTmbX = startX + width - (opt.DASH_RIGHT ? dashWidth : 0) - wsTmbWidth - spacing / 2;
            else
                wsTmbX = (opt.DASH_LEFT ? dashWidth : 0) + spacing / 2;
            ...
            let wsTmbY = startY + (dashHeightReservation && opt.DASH_TOP ? dashHeight : 0) + wstOffset;
        ...
        if (opt.CENTER_SEARCH_VIEW) {
            ...
            childBox.set_origin(Math.round(wsTmbWidth + dashW), Math.round(startY + (opt.DASH_TOP ? dashHeight : spacing) + searchHeight));
        } else {
            childBox.set_origin(Math.round(this._xAlignCenter ? wsTmbWidth + spacing : searchXoffset), Math.round(startY + (opt.DASH_TOP ? dashHeight : spacing) + searchHeight));
        }
...
const ControlsManagerLayoutHorizontal = {
    ...
    vfunc_allocate(container, box) {
        ...
        if (opt.SHOW_WS_TMB) {
            ...
            let wsTmbY;
            if (opt.WS_TMB_TOP)
                wsTmbY = startY + (opt.DASH_TOP ? dashHeight : spacing / 2);
            else
                wsTmbY = startY + height - (opt.DASH_BOTTOM ? dashHeight : 0) - wsTmbHeight;

            let wstOffset = (width - wsTmbWidth) / 2;
            wstOffset -= opt.WS_TMB_POSITION_ADJUSTMENT * (wstOffset - spacing / 2);
            let wsTmbX = Math.clamp(
                startX + wstOffset,
        ...
        if (opt.CENTER_SEARCH_VIEW) {
            ...
            childBox.set_origin(Math.round(dashW), Math.round(startY + (opt.DASH_TOP ? dashHeight : spacing) + (opt.WS_TMB_TOP ? wsTmbHeight + spacing : 0) + searchHeight));
        } else {
            childBox.set_origin(Math.round(this._xAlignCenter ? spacing : searchXoffset), Math.round(startY + (opt.DASH_TOP ? dashHeight : spacing) + (opt.WS_TMB_TOP ? wsTmbHeight + spacing : 0) + searchHeight));
        }
