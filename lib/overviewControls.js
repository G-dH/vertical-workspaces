...
const ControlsManagerCommon = {
    ...
    _updateBackground(bgManager, stateValue = 2) {
        ...
        if (!opt.SHOW_BG_IN_OVERVIEW && !opt.SHOW_WS_PREVIEW_BG) {
            this._fadeOutWallpaper(bgManager, stateValue, staticWorkspace);
        } else {
    ...
    _setBgBrightness(bgManager, stateValue, staticWorkspace, searchActive) {
        ...
        const overviewBrightness = staticWorkspace ? 1 : opt.OVERVIEW_BG_BRIGHTNESS;
        ...
        const vignette = staticWorkspace ? 0 : 0.2;

        let currentBrightness = 1;
        let currentVignette = 0;
        if (opt.SHOW_WS_PREVIEW_BG && stateValue < 1) {
        ...
        print(stateValue, currentBrightness, currentVignette);
    ...
    _setBlurEffect(bgManager, stateValue, staticWorkspace) {
        ...
        const overviewBlurRadius = staticWorkspace ? 0 : opt.OVERVIEW_BG_BLUR_SIGMA;
    ...
    _fadeOutWallpaper(bgManager, finalState, stateValue, staticWorkspace) {
        ...
        if (!(staticWorkspace && finalState === 1))
