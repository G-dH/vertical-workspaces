...
var OverviewControlsModule = class {
    ...
    _addClickToCloseOverview(reset) {
        ...
        const searchResults = controls._searchController._searchResults;

        controls.reactive = false;
        if (this._overviewClickAction && controls.get_actions().indexOf(this._overviewClickAction) !== -1)
            controls.remove_action(this._overviewClickAction);
        ...
        appDisplay.reactive = false;
        if (this._appDisplayClickAction && appDisplay.get_actions().indexOf(this._overviewClickAction) !== -1)
            appDisplay.remove_action(this._appDisplayClickAction);
        this._appDisplayClickAction = null;
        ...
        this._overviewClickAction.connect('clicked', () => {
            /* if (controls._stateAdjustment.value > ControlsState.WINDOW_PICKER)
                controls._shiftState(Meta.MotionDirection.DOWN);
            else */
            Main.overview.hide();
        });
        controls.reactive = true;
        controls.add_action(this._overviewClickAction);
        ...
        this._appDisplayClickAction = new Clutter.ClickAction();
        this._appDisplayClickAction.connect('clicked', () => Clutter.EVENT_STOP);
        appDisplay.reactive = true;
        appDisplay.add_action(this._appDisplayClickAction);
    }
};
...
const ControlsManagerLayoutVertical = {
    ...
    _getAppDisplayBoxForState(state, box, workAreaBox, searchHeight, dashWidth, dashHeight, thumbnailsWidth, startY) {
        ...
        const yOffsetT = (opt.DASH_TOP ? dashHeight : 0) + (opt.SHOW_SEARCH_ENTRY ? searchHeight : 0) + spacing;
        ...
        const appDisplayX = startX + (opt.CENTER_APP_GRID ? (width - adWidth) / 2 : xOffsetL + 2 * spacing);
...
const ControlsManagerLayoutHorizontal = {
    ...
    _getAppDisplayBoxForState(state, box, workAreaBox, searchHeight, dashWidth, dashHeight, thumbnailsHeight, startY) {
        ...
        const appDisplayX = startX + (opt.CENTER_APP_GRID ? (width - adWidth) / 2 : xOffsetL + hSpacing);
        const appDisplayY = startY + yOffsetT;
