...
const ControlsManagerCommon = {
    ...
    _updateSearchStyle(reset) {
        if (!reset && (((opt.OVERVIEW_MODE2 && !opt.WORKSPACE_MODE && !this.dash.showAppsButton.checked) || opt.SEARCH_RESULTS_BG_STYLE))) {
            this._searchController._searchResults.add_style_class_name('search-results-bg-om2');
            this._searchEntry.add_style_class_name('search-entry-om2');
        } else {
            this._searchController._searchResults.remove_style_class_name('search-results-bg-om2');
