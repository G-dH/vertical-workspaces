...
const AppSearchProvider = {
    getInitialResultSet(terms, cancellable) {
        ...
        // Remove all accents from the search terms
        let _terms = [];
        terms.forEach(t => _terms.push(t.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase()));

        // Detect supported prefix that can restrict search results
        const prefix = terms[0].slice(0, 2);
        let systemActionsOnly, settingsOnly, excludeSettingsApp = false;
        if (prefix === '..') {
            // Two dots .. prefix is used to restrict search to System Actions only
            systemActionsOnly = terms[0].slice(0, 2) === '..';
            // Remove prefix from search terms if needed
            _terms[0] = _terms[0].slice(2);
        } else if (opt.SEARCH_INCLUDE_SETTINGS && prefix === '--') {
            // Two dashes -- prefix is used to restrict search to Setting panels only
            settingsOnly = _terms[0].slice(0, 2) === '--';
            // Exclude the parent app launcher if user searches for a particular settings panel
            excludeSettingsApp = settingsOnly && _terms[0].length > 2;
            // Remove prefix from search terms
            _terms[0] = _terms[0].slice(2);
        }
        ...
        //      Search for terms in name and keywords
        ...
        //      Sort System Actions by relevance
        ...
        // Skip searching apps if user wants System Actions
        let appInfoList = systemActionsOnly
            ? []
            : Shell.AppSystem.get_default().get_installed();
        ...
        appInfoList = appInfoList.filter(appInfo => {
            ...
            if (appInfo.get_display_name) {
                ...
                // Settings app and Settings panel launchers
                const isSettingsLauncher = cmd?.includes('gnome-control-center');
                // For Settings app panel launchers appInfo.shouldShow() returns false
                const isSettingsApp = isSettingsLauncher && appInfo.should_show();

                // Show only launchers that should be visible in this DE, and Settings panel launchers if enabled
                shouldShow =
                    (appInfo.should_show() && this._parentalControlsManager.shouldShowApp(appInfo) && !settingsOnly) ||
                    (opt.SEARCH_INCLUDE_SETTINGS && isSettingsLauncher && !(excludeSettingsApp && isSettingsApp));
                ...
                if (shouldShow) {
                    ...
                    // In GNOME 49.beta Gio.DesktopAppInfo was missing some methods
                    // get_generic_name(), get_string(), ...
                    let gName = (appInfo.get_generic_name ? appInfo.get_generic_name() : '') || '';
                    let description = appInfo.get_description() || '';
                    let categories = appInfo.get_categories?.bind(appInfo)()?.replace(/;/g, ' ') || '';
                    let keywords = appInfo.get_keywords?.bind(appInfo)()?.join(' ') || '';
                    name = `${dispName} ${id}`;
                    let packageType = '';
                    ...
                    // Remove strings that make the search less efficient
                    if (settingsOnly) {
                        id = '';
                        cmd = '';
                        categories = '';
...
const SearchResult = {
    // Note that app search provider activates directly app icons
    activate() {
        ...
        if (!Me.Util.isShiftPressed())
            // Delay the transition to the desktop so the previous actions have time to complete
            GLib.idle_add(GLib.PRIORITY_DEFAULT_IDLE, () => Main.overview.toggle());
    },
};
