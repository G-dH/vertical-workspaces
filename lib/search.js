...
var SearchModule = class {
    ...
    _activateModule() {
        ...
        const appSearchDisplay = Main.overview._overview.controls._searchController._searchResults._providers.filter(p => p.id === 'applications')[0]?.display;
        if (appSearchDisplay)
    ...
    _disableModule() {
        ...
        const searchResults = searchController._searchResults;
        ...
        searchController.y_align = Clutter.ActorAlign.FILL;
...
const SearchResultsView = {
    ...
    _highlightFirstVisibleAppGridIcon() {
        const appDisplay = Main.overview._overview.controls._appDisplay;
        // Remove the "selected" style from previously selected icon, without knowing which one it is
        ...
        for (const icon of appDisplay._orderedItems) {
            if (icon.id === this._results.applications[0]) {
