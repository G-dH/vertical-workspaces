...
// direct imports are needed for class definitions
...
export const AppDisplayModule = class {
    ...
    _activateModule() {
        ...
        this._overrides.addOverride('BaseAppViewCommonApp', Ui.AppDisplay.AppDisplay.prototype, BaseAppViewCommon);
        this._overrides.addOverride('BaseAppViewCommonFolder', Ui.AppDisplay.FolderView.prototype, BaseAppViewCommon);
        ...
        if (opt.ORIENTATION === Gi.Clutter.Orientation.VERTICAL) {
            ...
            this._overrides.addOverride('BaseAppViewVerticalApp', Ui.AppDisplay.AppDisplay.prototype, BaseAppViewVertical);
            this._overrides.addOverride('BaseAppViewVerticalFolder', Ui.AppDisplay.FolderView.prototype, BaseAppViewVertical);
        ...
        const appDisplay = Ui.Main.overview._overview.controls._appDisplay;
        if (!this._originalWorkId)
            this._originalWorkId = appDisplay._redisplayWorkId;
        if (!this._newWorkId) {
            appDisplay._redisplayWorkId = Ui.Main.initializeDeferredWork(appDisplay, () => {
                appDisplay._redisplay();
                if (appDisplay._overviewHiddenId === 0)
                    appDisplay._overviewHiddenId = Ui.Main.overview.connect('hidden', () => appDisplay.goToPage(0));
            });
    ...
    _updateAppDisplayProperties(reset = false) {
        ...
        if (reset) {
            ...
            if (!this._appGridLayoutSettings) {
                ...
                this._appGridLayoutSettings = Misc.getSettings('org.gnome.shell');
...
function _getViewFromIcon(icon) {
    for (let parent = icon.get_parent(); parent; parent = parent.get_parent()) {
        if (parent instanceof Ui.AppDisplay.AppDisplay || parent instanceof Ui.AppDisplay.FolderView) {
            return parent;
        }
    }
    return null;
