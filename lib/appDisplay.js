...
var AppDisplayModule = class {
    ...
    _applyOverrides() {
        ...
        } else if (_appDisplay._appGridLayout) { // >= 43
...
const BaseAppViewCommon = {
    ...
    // GS 42 only, this fixes dnd from appDisplay to the workspace thumbnail on the left if appDisplay is on page 1 because of appgrid left overshoot
    ...
    _acceptDropCommon(source) {
        ...
        if ((Me.shellVersion < 43 && this._dropPage) ||
                (Me.shellVersion >= 43 &&
                (dropTarget === this._prevPageIndicator ||
...
const AppFolderDialog = {
    ...
    _updateFolderSize() {
        ...
        let verticalNavigation = opt.ORIENTATION ? navigationArrowsSize : pageIndicatorSize;
    ...
    _zoomAndFadeIn() {
        ...
        if (opt.APP_GRID_FOLDER_CENTER) {
            dialogTargetX = folderAreaX + folderAreaWidth / 2 - this.child.width / 2;
            dialogTargetY = folderAreaY + (folderAreaHeight / 2 - this.child.height / 2) / 2;
        } else {
            ...
            dialogTargetX = Math.clamp(
                dialogTargetX,
                folderAreaX,
                folderAreaX + folderAreaWidth - this.child.width
            );

            dialogTargetY = Math.clamp(
                dialogTargetY,
                folderAreaY,
                folderAreaY + folderAreaHeight - this.child.height
            );
        }
