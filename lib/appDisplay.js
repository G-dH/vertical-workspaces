...
var AppDisplayModule = class {
    constructor(gi, ui, misc, me) {
        // this.DIALOG_SHADE_NORMAL = Gi.Clutter.Color.from_pixel(0x00000022);
        // this.DIALOG_SHADE_HIGHLIGHT = Gi.Clutter.Color.from_pixel(0x00000000);
    ...
    _activateModule() {
        ...
        // Common
        this._overrides.addOverride('FolderView', Ui.AppDisplay.FolderView.prototype, FolderView);
        this._overrides.addOverride('FolderIcon', Ui.AppDisplay.FolderIcon.prototype, FolderIcon);
        this._overrides.addOverride('AppIcon', Ui.AppDisplay.AppIcon.prototype, AppIcon);
        this._overrides.addOverride('AppDisplay', Ui.AppDisplay.AppDisplay.prototype, AppDisplayCommon);
        this._overrides.addOverride('AppViewItem', Ui.AppDisplay.AppViewItem.prototype, AppViewItemCommon);
        this._overrides.addOverride('BaseAppViewCommon', Ui.AppDisplay.BaseAppView.prototype, BaseAppViewCommon);

        if (opt.ORIENTATION === Gi.Clutter.Orientation.VERTICAL) {
            this._overrides.addOverride('AppDisplayVertical', Ui.AppDisplay.AppDisplay.prototype, AppDisplayVertical);
            this._overrides.addOverride('BaseAppViewVertical', Ui.AppDisplay.BaseAppView.prototype, BaseAppViewVertical);
        }

        // Custom App Grid
        this._overrides.addOverride('AppFolderDialog', Ui.AppDisplay.AppFolderDialog.prototype, AppFolderDialog);
        if (Misc.shellVersion >= 43) {
            // const defined class needs to be touched before real access
            this._dummy = Ui.AppDisplay.AppGrid;
            delete this._dummy;
            // BaseAppViewGridLayout is not exported, we can only access current instance
            this._overrides.addOverride('BaseAppViewGridLayout', Ui.Main.overview._overview.controls._appDisplay._appGridLayout, BaseAppViewGridLayout);
            this._overrides.addOverride('FolderGrid', Ui.AppDisplay.FolderGrid.prototype, FolderGrid);
        } else {
            this._overrides.addOverride('FolderGrid', Ui.AppDisplay.FolderGrid.prototype, FolderGridLegacy);
        }

        this._setAppDisplayOrientation(opt.ORIENTATION === Gi.Clutter.Orientation.VERTICAL);
        this._updateDND();
        if (!Ui.Main.sessionMode.isGreeter)
    ...
    _setAppDisplayOrientation(vertical = false) {
        const CLUTTER_ORIENTATION = vertical ? Gi.Clutter.Orientation.VERTICAL : Gi.Clutter.Orientation.HORIZONTAL;
        ...
        let appDisplay = Ui.Main.overview._overview._controls._appDisplay;
        ...
        if (vertical) {
            appDisplay._scrollView.set_policy(Gi.St.PolicyType.NEVER, Gi.St.PolicyType.EXTERNAL);
            ...
            pageIndicators.y_align = Gi.Clutter.ActorAlign.CENTER;
            pageIndicators.x_align = Gi.Clutter.ActorAlign.START;
            ...
            appDisplay._scrollView.set_policy(Gi.St.PolicyType.EXTERNAL, Gi.St.PolicyType.NEVER);
            ...
            pageIndicators.y_align = Gi.Clutter.ActorAlign.END;
            pageIndicators.x_align = Gi.Clutter.ActorAlign.CENTER;
        ...
        if (!vertical) {
            ...
            Ui.Main.overview._overview._controls._appDisplay.scale_y = 1;
            Ui.Main.overview._overview._controls._appDisplay.scale_x = 1;
    ...
    _updateAppDisplayProperties(reset = false) {
        ...
        if (reset) {
            ...
            if (!this._appGridLayoutSettings) {
                this._appGridLayoutSettings = Misc.ExtensionUtils.getSettings('org.gnome.shell');
            ...
            if (!Ui.Main.layoutManager._startingUp) {
                this._updateAppGrid();
            } else if (Ui.Main.layoutManager._startingUp && (Gi.Meta.is_restart() || Me.Util.dashIsDashToDock())) {
                _timeouts.three = Gi.GLib.idle_add(Gi.GLib.PRIORITY_LOW, () => {
                    this._updateAppGrid();
                    _timeouts.three = 0;
    ...
    _updateDND(reset) {
        if (!reset) {
            if (!this._appSystemStateConId && opt.APP_GRID_INCLUDE_DASH >= 3) {
                this._appSystemStateConId = Gi.Shell.AppSystem.get_default().connect(
                    'app-state-changed',
                    () => {
                        this._updateFolderIcons = true;
                        Ui.Main.overview._overview.controls._appDisplay._redisplay();
                    }
                );
            }
        } else if (this._appSystemStateConId) {
            Gi.Shell.AppSystem.get_default().disconnect(this._appSystemStateConId);
            this._appSystemStateConId = 0;
        }
    }
    ...
    _restoreOverviewGroup() {
        ...
        Ui.Main.layoutManager.overviewGroup.opacity = 255;
        Ui.Main.layoutManager.overviewGroup.scale_x = 1;
        Ui.Main.layoutManager.overviewGroup.scale_y = 1;
        Ui.Main.layoutManager.overviewGroup.hide();
        Ui.Main.overview._overview._controls._appDisplay.translation_x = 0;
        Ui.Main.overview._overview._controls._appDisplay.translation_y = 0;
        Ui.Main.overview._overview._controls._appDisplay.visible = true;
        Ui.Main.overview._overview._controls._appDisplay.opacity = 255;
    }
    ...
    _removeIcons() {
        ...
        for (let i = 0; i < icons.length; i++) {
            const icon = icons[i];
            if (icon._dialog)
                Ui.Main.layoutManager.overviewGroup.remove_child(icon._dialog);
    ...
    _updateAppGrid(reset = false) {
        ...
        _timeouts.one = Gi.GLib.idle_add(Gi.GLib.PRIORITY_LOW, () => {
            ...
            // let the main loop process our changes before continuing
            _timeouts.two = Gi.GLib.idle_add(Gi.GLib.PRIORITY_LOW, () => {
    ...
    _exposeAppGrid() {
        const overviewGroup = Ui.Main.layoutManager.overviewGroup;
...
const AppDisplayVertical = {
    ...
    adaptToSize(width, height) {
        ...
        const adaptToSize = Ui.AppDisplay.BaseAppView.prototype.adaptToSize.bind(this);
        adaptToSize(width, height);
    },
};
...
const AppDisplayCommon = {
    ...
    _loadApps() {
        let appIcons = [];
        const runningApps = Gi.Shell.AppSystem.get_default().get_running().map(a => a.id);

        this._appInfoList = Gi.Shell.AppSystem.get_default().get_installed().filter(appInfo => {
            try {
        ...
        if (!opt.APP_GRID_USAGE) {
            ...
            folders.forEach(id => {
                ...
                if (!icon) {
                    icon = new Ui.AppDisplay.FolderIcon(id, path, this);
        ...
        apps.forEach(appId => {
            ...
            if (!icon) {
                ...
                icon = new Ui.AppDisplay.AppIcon(app, { isDraggable });
                icon.connect('notify::pressed', () => {
                    if (icon.pressed)
                        this.updateDragFocus(icon);
                });
    ...
    _onDragBegin(overview, source) {
        ...
        Ui.DND.addDragMonitor(this._dragMonitor);
        if (Misc.shellVersion < 43)
            this._slideSidePages(Ui.AppDisplay.SidePages.PREVIOUS | Ui.AppDisplay.SidePages.NEXT | Ui.AppDisplay.SidePages.DND);
        else
        ...
        // exist in Ui.AppDisplay. We work around that by adding a placeholder
        // icon that is either destroyed on cancel, or becomes the effective
        // new icon when dropped.
        if (Ui.AppDisplay._getViewFromIcon(source) instanceof Ui.AppDisplay.FolderView ||
    ...
    _ensurePlaceholder(source) {
        ...
        const appSys = Gi.Shell.AppSystem.get_default();
        ...
        this._placeholder = new Ui.AppDisplay.AppIcon(app, { isDraggable });
    ...
    acceptDrop(source) {
        ...
        if ((Misc.shellVersion < 43 && this._dropPage) ||
            (Misc.shellVersion >= 43 && (dropTarget === this._prevPageIndicator ||
            dropTarget === this._nextPageIndicator))) {
            let increment;

            if (Misc.shellVersion < 43)
                increment = this._dropPage === Ui.AppDisplay.SidePages.NEXT ? 1 : -1;
            else
        ...
        let view = Ui.AppDisplay._getViewFromIcon(source);
        if (view instanceof Ui.AppDisplay.FolderView)
...
const BaseAppViewVertical = {
    after__init() {
        this._grid.layoutManager._orientation = Gi.Clutter.Orientation.VERTICAL;
        this._scrollView.set_policy(Gi.St.PolicyType.NEVER, Gi.St.PolicyType.EXTERNAL);
        this._orientation = Gi.Clutter.Orientation.VERTICAL;
        this._swipeTracker.orientation = Gi.Clutter.Orientation.VERTICAL;
        this._swipeTracker._reset();
        this._pageIndicators.vertical = true;
        this._box.vertical = false;
        this._pageIndicators.x_expand = false;
        this._pageIndicators.y_align = Gi.Clutter.ActorAlign.CENTER;
        this._pageIndicators.x_align = Gi.Clutter.ActorAlign.START;
    ...
    _pageForCoords() {
        return Ui.AppDisplay.SidePages.NONE;
    },
};
...
const BaseAppViewCommon = {
    ...
    _redisplay() {
        ...
        const thisIsFolder = this instanceof Ui.AppDisplay.FolderView;
        ...
        (opt.APP_FOLDER_ORDER && thisIsFolder)) {
            ...
            if ((opt.APP_GRID_USAGE && thisIsAppDisplay) ||
                ...
                appIcons.sort((a, b) => Gi.Shell.AppUsage.get_default().compare(a.app.id, b.app.id));
            ...
            if (opt.APP_GRID_DASH_FIRST && thisIsAppDisplay)
                appIcons.sort((a, b) => a.app.get_state() !== Gi.Shell.AppState.RUNNING && b.app.get_state() === Gi.Shell.AppState.RUNNING);
    ...
    _canAccept(source) {
        return source instanceof Ui.AppDisplay.AppViewItem;
    },
    ...
    acceptDrop(source) {
        ...
        // _dropPage is not used from GS 43
        if (this._dropPage) {
            const increment = this._dropPage === Ui.AppDisplay.SidePages.NEXT ? 1 : -1;
            const { currentPage, nPages } = this._grid;
            const page = Math.min(currentPage + increment, nPages);
            const position = page < nPages ? -1 : 0;
    ...
    _onDragMotion(dragEvent) {
        if (!(dragEvent.source instanceof Ui.AppDisplay.AppViewItem))
        ...
        if (Misc.shellVersion < 43) {
            ...
            if (this._dropPage &&
               this._dropPage === Ui.AppDisplay.SidePages.PREVIOUS &&
        ...
        if (appIcon instanceof Ui.AppDisplay.AppViewItem) {
            if (Misc.shellVersion < 44) {
    ...
    adaptToSize(width, height, isFolder = false) {
        let box = new Gi.Clutter.ActorBox({
            x2: width,
            y2: height,
        });
        ...
        this._scrollView.content_padding = new Gi.Clutter.Margin({
            left: leftPadding,
            right: rightPadding,
            top: topPadding,
        ...
        this._pageArrowOffset = Math.max(
            leftPadding - 80, 0); // 80 is Ui.AppDisplay.PAGE_PREVIEW_MAX_ARROW_OFFSET
    },
};
    ...
    after__init() {
        ...
        // disable opening the folder with primary mouse button and enable the secondary one
         const buttonMask = opt.APP_GRID_ACTIVE_PREVIEW
            ? Gi.St.ButtonMask.TWO | Gi.St.ButtonMask.THREE
            : Gi.St.ButtonMask.ONE | Gi.St.ButtonMask.TWO;
        this.button_mask = buttonMask;*/
        this.button_mask = Gi.St.ButtonMask.ONE | Gi.St.ButtonMask.TWO;
    },
...
const FolderView = {
    _createGrid() {
        let grid = new Ui.AppDisplay.FolderGrid();
        return grid;
    },

    createFolderIcon(size) {
        const layout = new Gi.Clutter.GridLayout({
            row_homogeneous: true,
            column_homogeneous: true,
        });

        let icon = new Gi.St.Widget({
        ...
        let rtl = icon.get_text_direction() === Gi.Clutter.TextDirection.RTL;
        for (let i = 0; i < gridSize * gridSize; i++) {
            const style = `width: ${subSize}px; height: ${subSize}px;`;
            let bin = new Gi.St.Bin({ style, reactive: true });
            bin.pivot_point = new Gi.Graphene.Point({ x: 0.5, y: 0.5 });
            if (i < numItems) {
                if (!opt.APP_GRID_ACTIVE_PREVIEW) {
                    bin.child = this._orderedItems[i].app.create_icon_texture(subSize);
                } else {
                    const app = this._orderedItems[i].app;
                    const child = new Ui.AppDisplay.AppIcon(app, {
    ...
    _redisplay() {
        ...
        Ui.AppDisplay.BaseAppView.prototype._redisplay.bind(this)();
    },
    ...
    _loadApps() {
        ...
        appInfos.forEach(appInfo => {
            let appCategories = Ui.AppDisplay._getCategories(appInfo);
            if (!Ui.AppDisplay._listsIntersect(folderCategories, appCategories))
                return;
        ...
        this._apps.forEach(app => {
            ...
            if (!icon)
                icon = new Ui.AppDisplay.AppIcon(app);

            items.push(icon);
        });

        if (opt.APP_FOLDER_ORDER)
            Ui.Main.overview._overview.controls._appDisplay._sortOrderedItemsAlphabetically(items);

        if (opt.APP_FOLDER_USAGE)
            items.sort((a, b) => Gi.Shell.AppUsage.get_default().compare(a.app.id, b.app.id));
...
const FolderGridLegacy = {
    _init() {
        Ui.IconGrid.IconGrid.prototype._init.bind(this)({
            ...
            page_halign: Gi.Clutter.ActorAlign.CENTER,
            page_valign: Gi.Clutter.ActorAlign.CENTER,
        });
...
const FolderGrid = {
    _init() {
        Ui.AppDisplay.AppGrid.prototype._init.bind(this)({
            allow_incomplete_pages: false,
            columns_per_page: opt.APP_GRID_FOLDER_COLUMNS ? opt.APP_GRID_FOLDER_COLUMNS : 20,
            rows_per_page: opt.APP_GRID_FOLDER_ROWS ? opt.APP_GRID_FOLDER_ROWS : 20,
            page_halign: Gi.Clutter.ActorAlign.CENTER,
...
const FOLDER_DIALOG_ANIMATION_TIME = 200; // Ui.AppDisplay.FOLDER_DIALOG_ANIMATION_TIME
const AppFolderDialog = {
    ...
    after__init() {
        ...
        const clickAction = new Gi.Clutter.ClickAction();
        clickAction.connect('clicked', act => {
            if (act.get_button() === Gi.Clutter.BUTTON_PRIMARY)
                return Gi.Clutter.EVENT_STOP;
            const [x, y] = clickAction.get_coords();
            const actor = global.stage.get_actor_at_pos(Gi.Clutter.PickMode.ALL, x, y);
    ...
    after__addFolderNameEntry() {
        ...
        this._removeButton.connect('clicked', () => {
            if (Date.now() - this._removeButton._lastClick < Gi.Clutter.Settings.get_default().double_click_time) {
                ...
                let settings = new Gi.Gio.Settings({ schema_id: 'org.gnome.desktop.app-folders' });
    ...
    _updateFolderSize() {
        ...
        const appDisplayHeight = appDisplay.allocation.y2 - appDisplay.allocation.y1 + (opt.SHOW_SEARCH_ENTRY ? Ui.Main.overview._overview.controls._searchEntryBin.height : 0);
    ...
    _zoomAndFadeIn() {
        ...
        if (!opt.APP_GRID_FOLDER_CENTER) {
            ...
            const searchEntryHeight = opt.SHOW_SEARCH_ENTRY ? Ui.Main.overview._overview.controls._searchEntryBin.height : 0;
            dialogTargetX = appDisplay.x + appDisplay.width / 2 - this.child.width / 2;
            dialogTargetY = appDisplay.y - searchEntryHeight + (appDisplay.height + searchEntryHeight) / 2 - this.child.height / 2;
        }
        ...
        if (opt.SHOW_SEARCH_ENTRY) {
            Ui.Main.overview.searchEntry.ease({
                opacity: 0,
                duration: FOLDER_DIALOG_ANIMATION_TIME,
                mode: Gi.Clutter.AnimationMode.EASE_OUT_QUAD,
            });
    ...
    _setLighterBackground(lighter) {
        if (this._isOpen)
            Ui.Main.overview._overview._controls._appDisplay.opacity = lighter ? 20 : 0;
...
const AppIcon = {
    ...
    _canAccept(source) {
        ...
        return source !== this &&
               ...
               (view instanceof Ui.AppDisplay.AppDisplay &&
                !opt.APP_GRID_USAGE);
    },
};
...
const AppViewItemCommon = {
    _updateMultiline() {
        ...
        label.set_easing_duration(expand
            ? Ui.AppDisplay.APP_ICON_TITLE_EXPAND_TIME
            : Ui.AppDisplay.APP_ICON_TITLE_COLLAPSE_TIME);
        clutterText.set({
            line_wrap: expand,
            line_wrap_mode: expand ? Gi.Pango.WrapMode.WORD_CHAR : Gi.Pango.WrapMode.NONE,
            ellipsize: expand ? Gi.Pango.EllipsizeMode.NONE : Gi.Pango.EllipsizeMode.END,
        });
    },
...
const ActiveFolderIcon = {
    ...
    _onDragEnd() {
        ...
        Ui.Main.overview.endItemDrag(this._sourceItem.icon);
    },
};
