...
function _resetAppGrid(settings = null, key = null) {
    ...
    for (let i = items.length - 1; i > -1; i--) {
        const icon = items[i];
        try {
            // remove the icon from the grid, not destroy it
            Main.overview._overview._controls._appDisplay._removeItem(icon);
            if (icon._dialog) {
                // folder dialogs are placed in Main.layoutManager.overviewGroup
                // they must be removed because we don't know if they will be reused
                Main.layoutManager.overviewGroup.remove_actor(icon._dialog);
                icon._dialog = undefined;
            }
        } catch (e) {
            log(`Warning:${e}`);
        }
