    ...
    _loadApps() {
        ...
        if (!opt.APP_GRID_ORDER) {
            ...
            folders.forEach(id => {
                ...
                // for some reason this destroy() can crash GS 44
                ...
                // if we leave the icon live, it has no visible effects
                // better to waste some memory than crash the entire shell
                // to-do: find the cause of crashes
                if (!icon.visible) {
                    // icon.destroy();
                    return;
                }
