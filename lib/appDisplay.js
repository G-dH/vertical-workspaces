...
var AppDisplayModule = class {
    ...
    _activateModule() {
        ...
        if (!_appDisplay._onDragDrop) // GS 42 only
            _appDisplay._onDragDrop = () => {};
    ...
    _disableModule() {
        ...
        _appDisplay._scrollView.disconnectObject(this);
    ...
    _setAppDisplayOrientation(orientation) {
        ...
        if (orientation) {
            ...
            // 42 only - prevent closing overview when clicking navigation arrow widgets
            // if CLICK_EMPTY_CLOSE
            if (_appDisplay._onButtonPress) {
                _appDisplay._scrollView.disconnectObject(this);
                _appDisplay._scrollView.connectObject('button-press-event', (actor, event) => {
                    if (!opt.CLICK_EMPTY_CLOSE || !_appDisplay._grid.nPages)
                        return Clutter.EVENT_CONTINUE;
                    const page = _appDisplay._pageForCoords(...event.get_coords());
                    // This callback is called after the primary _onButtonPress and the page has been already switched
                    // so block the event it in any case
                    if (page === AppDisplay.SidePages.NEXT)
                        return Clutter.EVENT_STOP;
                    else if (page === AppDisplay.SidePages.PREVIOUS)
                        return Clutter.EVENT_STOP;
                    return Clutter.EVENT_CONTINUE;
                }, this);
            }
...
const AppDisplayCommon = {
    ...
    adaptToSize(width, height) {
        ...
        // grid calculation needs to set paddings first in adaptToSize
        adaptToSize(width, height);
    ...
    acceptDrop(source) {
        ...
        // When app is removed from the folder, we need to redisplay its grids
        // but skip redisplay of the main grid until the new icon position is stored
        this._readyToRedisplay = false;
        if (view instanceof AppDisplay.FolderView)
            view.removeApp(source.app);
        this._readyToRedisplay = true;
...
const BaseAppViewCommon = {
    ...
    _onDragMotion(dragEvent) {
        ...
        // Prevent reorganizing the main app grid icons when sorting is not custom
    ...
    adaptToSize(width, height, isFolder = false) {
        ...
        pageHeight = Math.ceil(availHeight * (isFolder ? 1 : opt.APP_GRID_PAGE_HEIGHT_SCALE));
        pageWidth = Math.ceil(availWidth * (isFolder ? 1 : opt.APP_GRID_PAGE_WIDTH_SCALE));

        this._grid.layout_manager.pagePadding.top = 30;
        this._grid.layout_manager.pagePadding.bottom = 30;
        this._grid.layout_manager.pagePadding.left = 30;
        this._grid.layout_manager.pagePadding.right = 30;
        ...
        // in 42 this padding needs to be used in grid calculation
        this._grid._contentPadding = this._scrollView.content_padding;
...
const AppFolderDialog = {
    ...
    vfunc_key_press_event(event) {
        ...
        case Clutter.KEY_Right:
            ...
            break;
        /* Backwards navigation just breaks the focus chain
        /* case Clutter.KEY_Up:
        ...
        return this._view._grid.navigate_focus(null, direction, false);
    },
};
