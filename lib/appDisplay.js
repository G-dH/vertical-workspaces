...
const AppFolderDialog = {
    ...
    _updateFolderSize() {
        ...
        while (height * scaleFactor > appDisplay.height + 100 - 2 * dialogMargin) {
            height -= itemSize + spacing;
            rows -= 1;
        }
        while (width * scaleFactor > appDisplay.width - 2 * dialogMargin) {
        ...
        if (fullAdaptive) {
            ...
            while ((nItems > columns * rows) && ((width * scaleFactor + itemSize + spacing) <= (appDisplay.width - 2 * dialogMargin))) {
                width += itemSize + spacing;
                columns += 1;
            }
            while ((nItems > columns * rows) && ((height * scaleFactor + itemSize + spacing) <= (appDisplay.height - 2 * dialogMargin))) {
                height += itemSize + spacing;
                rows += 1;
            }
        }
