...
function _updateAppGridProperties(reset = false) {
    ...
    // collect all icons created in the app grid so we can remove unstaged ones later
    // destroying app grid icons immediately after removing throws errors
    // to-do: needs better solution
    if (!appDisplay._iconCollector)
        appDisplay._iconCollector = [];
    ...
    if (reset) {
        ...
        if (appDisplay._destroyRemovedIconsTimeout)
            GLib.source_remove(appDisplay._destroyRemovedIconsTimeout);
        appDisplay._destroyRemovedIconsTimeout = undefined;
        appDisplay._iconCollector = undefined;
        appDisplay._grid.layoutManager.columnSpacing = 12;
        appDisplay._grid.layoutManager.rowSpacing = 12;
    } else {
        ...
        appDisplay._grid.layoutManager.columnSpacing = opt.APP_GRID_SPACING;
        appDisplay._grid.layoutManager.rowSpacing = opt.APP_GRID_SPACING;
...
const AppDisplayCommon = {
    ...
    _destroyGarbageIcons() {
        // destroying icons immediately after removing them from the grid causes access to dead objects errors
        // this is an workaround that keeps level of garbage low
        this._iconCollector.forEach((icon, index) => {
            if (!this._orderedItems.includes(icon)) {
                icon.destroy();
                this._iconCollector.splice(index, 1);
            }
        });
    },
    ...
    _redisplay() {
        ...
        if (this._destroyRemovedIconsTimeout)
            GLib.source_remove(this._destroyRemovedIconsTimeout);
        this._destroyRemovedIconsTimeout = GLib.timeout_add_seconds(
            0, 3,
            () => {
                this._destroyGarbageIcons();
                this._destroyRemovedIconsTimeout = 0;
...
const BaseAppViewCommon = {
    ...
    _redisplay() {
        ...
        removedApps.forEach(icon => {
            try {
                ...
                if (icon) {
                    const appDisplay = Main.overview._overview._controls._appDisplay;
                    const index = appDisplay._iconCollector.indexOf(icon);
                    if (index > -1)
                        appDisplay._iconCollector.splice(index, 1);
                    icon.destroy();
                }
            }
        });
    ...
    // adjustable page width gor GS 42 only
...
const AppFolderDialog = {
    ...
    _updateFolderSize(real = false) {
        ...
        if (real) {
            ...
            const [firstItemWidth, firstItemHeight] = firstItem.get_preferred_size();
            itemSize = Math.max(firstItemWidth, firstItemHeight) / scaleFactor;
        }
...
function _resetAppGrid(settings = null, key = null) {
    if (settings) {
        // redisplay only if all folders where removed by using reset button in Settings window
    ...
    for (let i = items.length - 1; i > -1; i--) {
        try {
            ...
            if (icon._dialog)
                Main.layoutManager.overviewGroup.remove_actor(icon._dialog);
        } catch (e) {
            log(`Warning: ${e}`);
        }
    }
