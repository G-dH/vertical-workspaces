...
export const AppDisplayModule = class {
    ...
    _updateAppDisplay(reset) {
        ...
        if (reset) {
            ...
            // Meta.is_restart() is related to X11 session and is no longer supported since GNOME 50
                ...
                _timeouts.three = GLib.idle_add(GLib.PRIORITY_DEFAULT_IDLE, () => {
                    this._repopulateAppDisplay();
                    _timeouts.three = 0;
            ...
            _appDisplay._pageIndicators.set_style(
                orientation
                    ? `margin-bottom: ${opt.APP_GRID_SPACING}px;`
                    : '');
        }
    }
    ...
    _realizeAppDisplay(callback) {
        ...
        _timeouts.updateAppGrid = GLib.idle_add(GLib.PRIORITY_DEFAULT_IDLE, () => {
...
const BaseAppViewCommon = {
    ...
    _adaptForOrientation(orientation, folder) {
        ...
        this._grid.set_style(
            `column-spacing: ${spacing}px;
            max-column-spacing: ${spacing}px;
            row-spacing: ${spacing}px;
            max-row-spacing: ${spacing}px;`
        );
    ...
    vfunc_allocate(container, box) {
        ...
        const arrowOffset = Math.round(
            this._grid._isFolder
                ...
                : (pageIndicatorsHeight - opt.APP_GRID_SPACING) / 2
        );
...
const FolderGrid = GObject.registerClass({
    ...
    _updatePadding() {
        ...
        padding.right = 0;
        padding.top = opt.APP_GRID_FOLDER_SPACING;
        padding.bottom = pageIndicatorSize + opt.APP_GRID_FOLDER_SPACING;
        this.layoutManager.pagePadding = padding;
    }
});
...
const AppFolderDialog = {
    ...
    _onRemoveFolder() {
        ...
        if (shouldRemoveFolder) {
            // popdown(callback)
            this.popdown(() => this._removeFolder());
