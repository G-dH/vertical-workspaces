...
function _updateAppGridProperties(reset = false) {
    ...
    if (reset) {
        ...
        // coverPane is active during startup animation, before is undefined, after is null
        if ((Main.layoutManager._coverPane === undefined && Meta.is_restart()) || Main.layoutManager._coverPane === null)
            _resetAppGrid();
    }
}
...
/**
 * Reset and update appDisplay
 *
 * @param {boolean} reset - reset to defaults
 * @param {boolean} realize - allows to disable pre-realization of the appDisplay
 * @param {boolean} shellInit - if true, Main.layoutManager.overviewGroup shouldn't be touched during appDisplay pre-realization (call from startupAnimation)
 */

function _resetAppGrid(reset = false, shellInit = false) {
    const appDisplay = Main.overview._overview._controls._appDisplay;
    ...
    if ((shellInit !== true &&
        (Main.layoutManager._coverPane === undefined && Meta.is_restart())) || (Main.layoutManager._coverPane === null && shellInit !== true)) {
        // scale down the overviewGroup so it don't cover uiGroup
        Main.layoutManager.overviewGroup.scale_y = 0.001;
    ...
    // make the app grid also visible
    appDisplay.visible = true;
    ...
    GLib.idle_add(GLib.PRIORITY_LOW, () => {
        ...
        // move app grid to the visible area
        ...
        GLib.idle_add(GLib.PRIORITY_LOW, () => {
            ...
            appDisplay.visible = false;
            appDisplay.opacity = 255;
            if (Main.layoutManager.overviewGroup.scale_y < 1)
                Main.layoutManager.overviewGroup.hide();
            Main.layoutManager.overviewGroup.scale_y = 1;
            Main.layoutManager.overviewGroup.opacity = 255;
...
const AppFolderDialog = {
    ...
    _updateFolderSize() {
        ...
        if (!appDisplay.width || appDisplay.allocation.x2 === Infinity) {
            return;
        }
        ...
        const fullAdaptive = !columns && !rows;
        ...
        let itemSize = iconSize + 51; // icon padding
        ...
        const appDisplayHeight = appDisplay.allocation.y2 - appDisplay.allocation.y1;
