...
const AppFolderDialog = {
    ...
    _updateFolderSize() {
        ...
        if (this._notFirstRun) {
            ...
            const realSize = Math.max(firstItemWidth, firstItemHeight) / scaleFactor;
        ...
        let height = rows * (itemSize + spacing) + /* header*/90 + /* padding*/100;
...
function _removeIcons() {
    ...
    const icons = [...appDisplay._orderedItems];
    for (let i = 0; i < icons.length; i++)
        appDisplay._removeItem(icons[i]);
}
