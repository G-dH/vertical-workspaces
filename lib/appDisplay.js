...
const AppFolderDialog = {
    ...
    _zoomAndFadeIn() {
        ...
        if (opt.APP_GRID_FOLDER_CENTER) {
            ...
            const minX = Math.min(folderAreaX, folderAreaX + (folderAreaWidth - folder.width) / 2);
            const maxX = Math.max(folderAreaX + folderAreaWidth - folder.width, folderAreaX + folderAreaWidth / 2 - folder.width / 2);
            const minY = Math.min(folderAreaY, folderAreaY + (folderAreaHeight - folder.height) / 2);
            const maxY = Math.max(folderAreaY + folderAreaHeight - folder.height, folderAreaY + folderAreaHeight / 2 - folder.height / 2);
...
const AppIcon = {
    after__init() {
        ...
        if (opt.APP_GRID_SHOW_PACKAGE_BADGE)
            this._addSourceIcon();
    },
    ...
    _addSourceIcon() {
        ...
        let cmd = this.app.get_app_info().get_commandline();
        let packageType;
        ...
        if (this.icon.label?.get_text().includes('Extension Manager'))
            packageType = 'snap';

        if (this.icon.label?.get_text().includes('Brave'))
            packageType = 'appimage';
        ...
        const sourceIconSize = 16;
        const icon = new St.Icon({
            gicon,
            icon_size: sourceIconSize,
            icon_name: packageType,
            x_expand: true,
            y_expand: true,
            x_align: Clutter.ActorAlign.END,
            y_align: Clutter.ActorAlign.START,
        });
        icon.translation_x = 8;
        icon.translation_y = Main.overview._overview.controls._appDisplay._grid.layoutManager._iconSize - sourceIconSize;
        if (opt.APP_GRID_SHOW_PACKAGE_BADGE === 1)
            icon.add_style_class_name('source-badge-selected-only');
        else
            icon.add_style_class_name('source-badge');
