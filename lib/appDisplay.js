...
export const AppDisplayModule = class {
    ...
    _activateModule() {
        ...
        // AppDisplay.BaseAppView.prototype is not exported so we have to override inherited classes
        ...
        this._overrides.addOverride('BaseAppViewGridLayout', _appDisplay._appGridLayout, BaseAppViewGridLayout);
        ...
        if (!this._originalWorkId)
            this._originalWorkId = _appDisplay._redisplayWorkId;
        if (!this._newWorkId) {
            _appDisplay._redisplayWorkId = Main.initializeDeferredWork(_appDisplay, () => {
                _appDisplay._redisplay();
                if (_appDisplay._overviewHiddenId === 0)
                    _appDisplay._overviewHiddenId = Main.overview.connect('hidden', () => _appDisplay.goToPage(0));
            });
            this._newWorkId = _appDisplay._redisplayWorkId;
        } else {
            _appDisplay._redisplayWorkId = this._newWorkId;
        }
    ...
    _disableModule() {
        ...
        _appDisplay._redisplayWorkId = this._originalWorkId;
    ...
    _setAppDisplayOrientation(vertical = false) {
        ...
        _appDisplay._orientation = CLUTTER_ORIENTATION;
        _appDisplay._grid.layoutManager._orientation = CLUTTER_ORIENTATION;
        _appDisplay._swipeTracker.orientation = CLUTTER_ORIENTATION;
        ...
        _appDisplay._adjustment = vertical
        ...
        if (!vertical) {
            ...
            _appDisplay.scale_y = 1;
            _appDisplay.scale_x = 1;
...
const FolderGrid = GObject.registerClass({
    ...
    _init() {
        ...
        this.setGridModes([
            {
                columns: /* opt.APP_GRID_FOLDER_COLUMNS ? opt.APP_GRID_FOLDER_COLUMNS :*/ 20,
                rows: /* opt.APP_GRID_FOLDER_ROWS ? opt.APP_GRID_FOLDER_ROWS :*/ 20,
            },
        ]);
    }
...
const AppFolderDialog = {
    ...
    _updateFolderSize() {
        ...
        const minDialogWidth =
            Math.round(this._entryBox.get_preferred_width(-1)[1] + 2 * margin) / scaleFactor;
        ...
        this.child.set_style(`
            ...
            height: ${height}px;
        `);
        // Setting the size only using the style leads to unexpected size changes
        // when grid is set to 6x3, for some reason
        // The first allocation is OK, but any further interaction with the dialog
        // makes the dialog insanely big
