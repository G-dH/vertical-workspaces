    ...
    _redisplay() {
        ...
        newApps.forEach(icon => {
            ...
            if (addedApps.includes(icon)) {
                if (icon.app?.get_name() === 'Quadrapassel')
...
const AppFolderDialog = {
    ...
    _updateFolderSize() {
        ...
        // adaptive icon size will be calculated for this dimensions
        width = Math.min(monitor.width, width);
        width = Math.max(540, width);
        height = Math.min(monitor.height, height);
