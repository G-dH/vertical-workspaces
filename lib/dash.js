...
const shellVersion46 = !Main.overview.dash.add_actor;
...
export const DashModule = class {
    ...
    _activateModule() {
        ...
        if (opt.DASH_VERTICAL) {
            ...
            if (shellVersion46)
                dash.add_style_class_name('vertical-46');
            else
    ...
    _setOrientation(orientation, dash) {
        ...
        if (orientation && opt.DASH_BG_GS3_STYLE) {
            if (opt.DASH_LEFT) {
                if (shellVersion46)
                    dash.add_style_class_name('vertical-46-gs3-left');
                else
                    dash.add_style_class_name('vertical-gs3-left');
            } else if (opt.DASH_RIGHT) {
                if (shellVersion46)
                    dash.add_style_class_name('vertical-46-gs3-right');
                else
    ...
    _updateSearchWindowsIcon(show = opt.SHOW_WINDOWS_ICON, dash) {
        ...
        if (!show || !(opt.get('windowSearchProviderModule') || Me.Util.getEnabledExtensions('windows-search-provider').length))
            return;
    ...
    _updateExtensionsIcon(show = opt.SHOW_EXTENSIONS_ICON, dash) {
        ...
        if (!show || !(opt.get('extensionsSearchProviderModule') || Me.Util.getEnabledExtensions('extensions-search-provider').length))
            return;
...
const DashItemContainerCommon = {
    ...
    showLabel() {
        ...
        if (opt.DASH_TOP) {
            const yOffset = itemHeight + (shellVersion46 ? 8 : -3);
            y = stageY + yOffset;
    ...
    _updateRunningDotStyle() {
        ...
        if (!this.label && shellVersion46) {
            if (opt.DASH_VERTICAL) {
                ...
                this._dot.translation_x = opt.DASH_LEFT ? -6 : 6;
            } else {
...
const ShowRecentFilesIcon = {
    _afterInit() {
        this._labelText = _('Search Recent Files (Hotkey: Shift + Space)');
        this.toggleButton = new St.Button({
            style_class: 'show-apps',
            track_hover: true,
            can_focus: true,
...
const ShowExtensionsIcon = {
    _afterInit() {
        this._labelText = _('Search Extensions (Hotkey: Ctrl + Space)');
        this.toggleButton = new St.Button({
            style_class: 'show-apps',
            track_hover: true,
            can_focus: true,
