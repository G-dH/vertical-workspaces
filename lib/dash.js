...
export const DashModule = class {
    ...
    _setOrientation(orientation, dash) {
        ...
        if (orientation && opt.DASH_BG_GS3_STYLE) {
            if (opt.DASH_LEFT) {
                if (shellVersion46) {
                    // move the container sou the dash items are clickable from the screen edge
    ...
    _updateSearchWindowsIcon(show = opt.SHOW_WINDOWS_ICON, dash) {
        ...
        if (!show || !Me.Util.getEnabledExtensions('windows-search-provider').length)
            return;
    ...
    _updateExtensionsIcon(show = opt.SHOW_EXTENSIONS_ICON, dash) {
        ...
        if (!show || !Me.Util.getEnabledExtensions('extensions-search-provider').length)
            return;
