...
const { Clutter, GObject, GLib, St, Shell, Meta } = imports.gi;
...
function update(reset = false) {
    ...
    if (conflict)
        log(`[${Me.metadata.name}] Dash module suspended due to potential conflict with another extension`);

    reset = reset || !moduleEnabled || conflict;

    // don't even touch this module if disabled or overridden by another extension
    if (_firstRun && reset)
        return;
