...
const DashIconCommon = {
    ...
    _updateRunningDotStyle() {
        ...
        // Compensate the 1px border if it's added to the dash background
        const offsetY = this._dot.get_theme_node().get_length('offset-y');
        if (opt.DASH_BG_COLOR) {
            // Ubuntu's GNOME 49 is styled differently and doesn't need this compensation
            if (offsetY < 0)
                this._dot.translationY = offsetY - 1;
        } else {
