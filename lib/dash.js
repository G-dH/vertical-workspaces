...
export const DashModule = class {
    ...
    _activateModule() {
        ...
        if (dash._showWindowsIcon && !dash._showWindowsIconClickedId)
            dash._showWindowsIconClickedId = dash._showWindowsIcon.toggleButton.connect('clicked', (a, c) => c && Me.Util.activateSearchProvider(Me.WSP_PREFIX));

        if (dash._recentFilesIcon && !dash._recentFilesIconClickedId)
            dash._recentFilesIconClickedId = dash._recentFilesIcon.toggleButton.connect('clicked', (a, c) => c && Me.Util.activateSearchProvider(Me.RFSP_PREFIX));
...
const DashOverride = {
    handleDragOver(source, actor, _x, y, _time) {
        ...
        if (pos !== this._dragPlaceholderPos && this._animatingPlaceholdersCount === 0) {
            ...
            this._dragPlaceholder = new Ui.Dash.DashItemContainer();
            this._dragPlaceholder.setChild(new Gi.St.Bin({ style_class: 'placeholder' }));
