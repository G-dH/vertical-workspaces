...
export var BaseIconSizes = [16, 24, 32, 40, 44, 48, 56, 64, 72, 80, 96, 112, 128];

let RecentFilesSearchProviderPrefix;// = Me.imports.lib.recentFilesSearchProvider.prefix;
let WindowSearchProviderPrefix;// = Me.imports.lib.windowSearchProvider.prefix;

const DASH_ITEM_LABEL_SHOW_TIME = 150;

export var DashModule = class {
    constructor(gi, ui, misc, me) {
        ...
        WindowSearchProviderPrefix = Me.WindowSearchProvider.prefix;
        RecentFilesSearchProviderPrefix = Me.RecentFilesSearchProvider.prefix;
    ...
    _activateModule() {
        ...
        if (opt.DASH_VERTICAL) {
            this._overrides.addOverride('Dash', Ui.Dash.Dash.prototype, DashOverride);
            this._setToVertical();
            dash.add_style_class_name('vertical');

            if (!this._verticalWorkId) {
                this._horizontalWorkId = dash._workId;
                dash._workId = Ui.Main.initializeDeferredWork(dash._box, dash._redisplay.bind(dash));
        ...
        if (dash._showWindowsIcon && !dash._showWindowsIconClickedId)
            dash._showWindowsIconClickedId = dash._showWindowsIcon.toggleButton.connect('clicked', (a, c) => c && Me.Util.activateSearchProvider(WindowSearchProviderPrefix));

        if (dash._recentFilesIcon && !dash._recentFilesIconClickedId)
            dash._recentFilesIconClickedId = dash._recentFilesIcon.toggleButton.connect('clicked', (a, c) => c && Me.Util.activateSearchProvider(RecentFilesSearchProviderPrefix));
    ...
    _setToVertical() {
        let dash = Ui.Main.overview._overview._controls.layoutManager._dash;

        dash._box.layout_manager.orientation = Gi.Clutter.Orientation.VERTICAL;
        dash._dashContainer.layout_manager.orientation = Gi.Clutter.Orientation.VERTICAL;
        dash._dashContainer.y_expand = false;
        dash._dashContainer.x_expand = true;
        dash.x_align = Gi.Clutter.ActorAlign.START;
        dash.y_align = Gi.Clutter.ActorAlign.CENTER;
        ...
        sizerBox.add_constraint(new Gi.Clutter.BindConstraint({
            ...
            coordinate: Gi.Clutter.BindCoordinate.WIDTH,
        }));
        sizerBox.add_constraint(new Gi.Clutter.BindConstraint({
            source: dash._dashContainer,
            coordinate: Gi.Clutter.BindCoordinate.HEIGHT,
        }));
    ...
    _setToHorizontal() {
        ...
        dash._box.layout_manager.orientation = Gi.Clutter.Orientation.HORIZONTAL;
        dash._dashContainer.layout_manager.orientation = Gi.Clutter.Orientation.HORIZONTAL;
        dash._dashContainer.y_expand = true;
        dash._dashContainer.x_expand = false;
        dash.x_align = Gi.Clutter.ActorAlign.CENTER;
        dash.y_align = 0;
    ...
    _connectShowAppsIcon(reset = false) {
        if (!reset) {
            ...
            Ui.Main.overview.dash._showAppsIcon.reactive = true;
            this._showAppsIconBtnPressId = Ui.Main.overview.dash._showAppsIcon.connect('button-press-event', (actor, event) => {
                const button = event.get_button();
                if (button === Gi.Clutter.BUTTON_MIDDLE)
                    Me.Util.openPreferences();
                else if (button === Gi.Clutter.BUTTON_SECONDARY)
                    Me.Util.activateSearchProvider(WindowSearchProviderPrefix);
                else
                    return Gi.Clutter.EVENT_PROPAGATE;
                return Gi.Clutter.EVENT_STOP;
            });
        } else if (this._showAppsIconBtnPressId) {
            Ui.Main.overview.dash._showAppsIcon.disconnect(this._showAppsIconBtnPressId);
            this._showAppsIconBtnPressId = 0;
            Ui.Main.overview.dash._showAppsIcon.reactive = false;
        }
    }
...
function getAppFromSource(source) {
    if (source instanceof Ui.AppDisplay.AppIcon)
        return source.app;
    else
        return null;
}
...
const Common = {
    ...
    switchWindow(direction) {
        ...
        // const focusWindow = Ui.AltTab.getWindows(null)[0]; // incompatible 45
