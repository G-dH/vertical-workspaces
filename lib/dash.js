...
const AppIconCommon = {
    ...
    activate(button) {
        ...
        } else if (showWidowsBeforeActivation && !isShiftPressed && nWindows > 1 && !targetWindowOnCurrentWs/* && !(opt.OVERVIEW_MODE && !opt.WORKSPACE_MODE)*/) {
            Main.wm.actionMoveWorkspace(appRecentWorkspace);
            Main.overview.dash.showAppsButton.checked = false;
            // Activate the last used app window to ensure it appears above all other apps in static workspace mode
        ...
        } else if (showWidowsBeforeActivation && opt.OVERVIEW_MODE2 && !opt.WORKSPACE_MODE && !isShiftPressed && nWindows > 1) {
            // expose windows
            Main.overview._overview._controls._thumbnailsBox._activateThumbnailAtPoint(0, 0, global.get_current_time(), true);
            return;
        } else if (((opt.DASH_SHIFT_CLICK_MV && isShiftPressed) || ((opt.DASH_CLICK_PREFER_WORKSPACE || opt.DASH_ISOLATE_WS) && !openNewWindow)) && nWindows > 0) {
    ...
    popupMenu(side = St.Side.LEFT) {
        ...
        if (this.app.state === Shell.AppState.RUNNING && this.app.get_n_windows()) {
