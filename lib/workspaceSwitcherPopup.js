...
export var WorkspaceSwitcherPopupModule = class {
    ...
    update(reset) {
        this._moduleEnabled = opt.get('workspaceSwitcherPopupModule');
        ...
        if (conflict && !reset)
            log(`[${Me.metadata.name}] Warning: "WorkspaceSwitcherPopup" module disabled due to potential conflict with another extension`);
    ...
    _activateModule() {
        ...
        this._overrides.addOverride('WorkspaceSwitcherPopup', Ui.WorkspaceSwitcherPopup.WorkspaceSwitcherPopup.prototype, WorkspaceSwitcherPopupCommon);
    }
...
const WorkspaceSwitcherPopupCommon = {
    ...
    _setPopupPosition() {
        ...
        if (opt.WS_SW_POPUP_MODE === 1) {
            ...
            workArea = global.display.get_monitor_geometry(Ui.Main.layoutManager.primaryIndex);
        } else {
