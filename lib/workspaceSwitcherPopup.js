...
const WorkspaceSwitcherPopupMod = {
    ...
    // However, _init() is called instead of constructor() if exists
    constructor() {
        this.constructor();
    },
    ...
    _init() {
        ...
        if (Meta.prefs_get_workspaces_only_on_primary() || !opt.WS_SW_POPUP_MODE) {
            this.add_child(new MonitorWorkspaceSwitcherPopup(global.display.get_primary_monitor()));
        } else {
            const monitors = Main.layoutManager.monitors;
            monitors.forEach((_, index) => {
                this.add_child(new MonitorWorkspaceSwitcherPopup(index));
            });
        }
    ...
    display(activeWorkspaceIndex) {
        // Ignore if the popup is disabled
