...
const IconGridCommon = {
    ...
    _findBestModeForSize(width, height) {
        ...
        if (iconSize > 0 && this._gridModes.length > 1) {
            ...
            if (!columns)
                columns = Math.floor(width / (iconSize + padding + spacing));
            if (!rows)
                rows = Math.floor(height / (iconSize + padding + spacing));
...
const IconGridLayoutCommon = {
    _findBestIconSize() {
        ...
        // reduce spacing in calculation so bigger icons have better chance to be selected
        // in most situations bigger icon fits better than smaller one
        const columnSpacingPerPage = this.columnSpacing * (nColumns - 1) / 2;
        const rowSpacingPerPage = this.rowSpacing * (nRows - 1) / 2;
        ...
        for (const size of iconSizes) {
            ...
            if (firstItem) {
                ...
                usedWidth = itemSize * nColumns; // + columnSpacingPerPage;
                usedHeight = itemSize * nRows; // + rowSpacingPerPage;
            } else {
