...
var PanelModule = class {
    ...
    _setPanelPosition(reset = false) {
        ...
        const panelBox = Main.layoutManager.panelBox;
        const panelHeight = Main.panel.height + 1; // panelBox height can be 0 after shell start
    ...
    _showPanel(show = true) {
        const panelBox = Main.layoutManager.panelBox;
        const panelHeight = Main.panel.height + 1;
        ...
        if (show) {
            panelBox.translation_y = opt.PANEL_POSITION_TOP ? -panelHeight + 1 : panelHeight - 1;
            Main.panel.opacity = 255;
            let delay = 0;
