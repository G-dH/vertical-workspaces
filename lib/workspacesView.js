...
const WorkspacesViewCommon = {
    ...
    _getWorkspaceModeForOverviewState(state, fullTransition, primaryMonitor) {
        switch (state) {
        ...
        case ControlsState.APP_GRID:
            if (!primaryMonitor)
                return opt.WORKSPACE_MODE;
            else if (!opt.WS_ANIMATION && !fullTransition)
                return opt.WORKSPACE_MODE;
            else
                return 0;
        }
    ...
    _updateVisibility() {
        // Visibility updates _updateWorkspacesState()
    ...
    _updateWorkspacesState() {
        ...
        const primaryMonitor = this._monitorIndex === Main.layoutManager.primaryMonitor.index;

        const fullTransition = primaryMonitor && Math.abs(finalState - initialState) === 2;
        const workspaceMode = (1 - fitMode) * Util.lerp(
            this._getWorkspaceModeForOverviewState(initialState, fullTransition, primaryMonitor),
            this._getWorkspaceModeForOverviewState(finalState, fullTransition, primaryMonitor),
            progress);
        ...
        this._workspaces.forEach((w, index) => {
            ...
            w.visible = opt.WS_ANIMATION_ALL ||
                ...
                (!primaryMonitor && distanceToCurrentWorkspace <= opt.NUMBER_OF_VISIBLE_NEIGHBORS) ||
            ...
            if (!primaryMonitor && !opt.SHOW_WS_PREVIEW_BG && currentState <= ControlsState.WINDOW_PICKER && w._background.opacity)
...
const WorkspacesDisplayCommon = {
    ...
    _onKeyPressEvent(actor, event) {
        ...
        case Clutter.KEY_ISO_Left_Tab:
            ...
            // The following code is common for all Arrow keys without and Tab
