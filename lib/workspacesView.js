    ...
    _updateWorkspacesState() {
        ...
        this._workspaces.forEach((w, index) => {
            ...
            w.visible = (this._animating && wsScrollProgress && distanceToCurrentWorkspace <= (opt.NUMBER_OF_VISIBLE_NEIGHBORS + 1)) || scaleProgress === 1 ||
                    (opt.WORKSPACE_MAX_SPACING >= opt.WS_MAX_SPACING_OFF_SCREEN && distanceToCurrentWorkspace <= opt.NUMBER_OF_VISIBLE_NEIGHBORS && currentState === ControlsState.WINDOW_PICKER) ||
                    ...
                    (opt.WORKSPACE_MAX_SPACING < opt.WS_MAX_SPACING_OFF_SCREEN && (!opt.FORCE_SINGLE_WS_TRANSITION ? true : distanceToCurrentWorkspace <= opt.NUMBER_OF_VISIBLE_NEIGHBORS && currentState <= ControlsState.WINDOW_PICKER &&
...
const SecondaryMonitorDisplayVertical = {
    ...
    _getWorkspacesBoxForState(state, box, wsTmbWidth, spacing) {
    ...
    vfunc_allocate(box) {
        ...
        let workspacesBox;
        const workspaceParams = [workArea, wsTmbWidth, spacing];
        if (!transitioning) {
            workspacesBox =
...
const SecondaryMonitorDisplayHorizontal = {
    ...
    _getWorkspacesBoxForState(state, box, wsTmbHeight, spacing) {
        const workspaceBox = box.copy();
    ...
    vfunc_allocate(box) {
        ...
        let workspacesBox;
        const workspaceParams = [workArea, wsTmbHeight, spacing];
        if (!transitioning) {
            workspacesBox =
                this._getWorkspacesBoxForState(currentState, ...workspaceParams);
        } else {
