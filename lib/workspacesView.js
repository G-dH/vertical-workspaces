        ...
        adjustments.forEach(adj => {
            if (adj.value === 0) {
                adj.ease(1, {
                    duration: 200,
                    mode: Clutter.AnimationMode.EASE_OUT_QUAD,
                    onComplete: () => Me.Util.activateKeyboardForWorkspaceView(),
                });
            }
        });
    },
...
const WorkspacesDisplayCommon = {
    ...
    _onKeyPressEvent(actor, event) {
        ...
        case Clutter.KEY_ISO_Left_Tab:
            if ((symbol === Clutter.KEY_Tab || symbol === Clutter.KEY_ISO_Left_Tab) && Me.Util.isSuperPressed(event.get_state()) && global.display.get_n_monitors() === 1) {
