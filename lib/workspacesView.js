...
var WorkspacesViewModule = class {
    constructor(gi, ui, misc, me) {
        ...
        this.dummy = Ui.WorkspacesView.SecondaryMonitorDisplay;
    ...
    _activateModule() {
        ...
        this._overrides.addOverride('SecondaryMonitorDisplayCommon', Ui.WorkspacesView.SecondaryMonitorDisplay.prototype, SecondaryMonitorDisplayCommon);
...
const SecondaryMonitorDisplayVertical = {
    ...
    _getWorkspacesBoxForState(state, box, padding, thumbnailsWidth, spacing) {
        ...
        case ControlsState.APP_GRID:
            ...
            yShift = 0;
            if (opt.SEC_WS_PREVIEW_SHIFT  && Ui.Main.panel.visible) {
    ...
    vfunc_allocate(box) {
        ...
        let thumbnailsWidth, thumbnailsHeight;
        this._thumbnails.visible = !opt.SEC_WS_TMB_HIDDEN;
        if (this._thumbnails.visible) {
            const reduceBoxHeight = opt.SEC_WS_PREVIEW_SHIFT && Ui.Main.panel.visible ? Ui.Main.panel.height : 0;
            ...
            wsTmbY += opt.SEC_WS_PREVIEW_SHIFT && Ui.Main.panel.visible ? Ui.Main.panel.height : 0;
...
const SecondaryMonitorDisplayHorizontal = {
    ...
    vfunc_allocate(box) {
        ...
        this._thumbnails.visible = !opt.SEC_WS_TMB_HIDDEN;
        if (this._thumbnails.visible) {
            const reservedHeight = opt.SEC_WS_PREVIEW_SHIFT && Ui.Main.panel.visible ? Ui.Main.panel.height : 0;
    ...
    _onScrollEvent(actor, event) {
        ...
        if (Me.Util.isShiftPressed()) {
            ...
            if (direction) {
                ...
                Ui.Main.overview._overview._controls._workspacesDisplay._workspacesViews[primaryMonitorIndex]._workspaces.forEach(w => {
                    w.visible = true;
                });
                return Gi.Clutter.EVENT_STOP;
            }
