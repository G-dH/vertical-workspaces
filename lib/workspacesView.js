...
const { GObject, Clutter, Meta, St, Graphene } = imports.gi;
    ...
    _updateWorkspacesState() {
        ...
        this._workspaces.forEach((w, index) => {
            ...
            if (!w.visible && distanceToCurrentWorkspace === 1 && initialState === ControlsState.APP_GRID && currentState === ControlsState.WINDOW_PICKER) {
                w.visible = true;
                const distanceUp = distance > 0;
                if (scaleX !== 1) {
                    const width = w.width * 0.6 * opt.WS_PREVIEW_SCALE;
                    w.translation_x = distanceUp ? -width : width;
                }
                if (scaleY !== 1) {
                    const height = Main.overview._overview._controls._workspacesDisplay.height;
                    w.translation_y = distanceUp ? -height : height;
                }
                // w.scale_x = scaleX;
                // w.scale_y = scaleY;
                w.opacity = 10;
