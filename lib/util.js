...
export function activateKeyboardForWorkspaceView() {
    Me.opt.showingPointerX = 0;

    if (!Me.opt.OVERVIEW_SELECT_WINDOW)
        return;

    const wsWindows = global.display.get_tab_list(0, global.workspace_manager.get_active_workspace());
    if (!wsWindows.length)
        return;
    ...
    for (let monitor = 0; monitor < nMonitors; monitor++) {
        ...
        let windowActor;
        if (Me.opt.OVERVIEW_SELECT_FIRST_WINDOW)
            windowActor = windows[0]._windowActor;
        if (Me.opt.OVERVIEW_SELECT_CURRENT_WINDOW)
