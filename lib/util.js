    ...
    windows.forEach(win => {
        if (win.get_monitor() !== targetMonitor) {
            ...
            GLib.idle_add(GLib.PRIORITY_DEFAULT_IDLE, () => {
                ...
                _removeMoveWinPreviewTimeout();
                Me.run.moveWindowPreviewTimeout = GLib.timeout_add(GLib.PRIORITY_DEFAULT_IDLE, 100, () => {
...
export function exposeWindows() {
    ...
    if (Me.run.initialPointerX === global.get_pointer()[0] && Me.opt.OVERVIEW_SELECT_WINDOW) {
        ...
        Me.run.winSpreadTimeout = GLib.timeout_add(GLib.PRIORITY_DEFAULT_IDLE, 250 * slowDownFactor,
            () => {
