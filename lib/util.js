...
export function osIsUbuntu() {
    return GLib.get_os_info('ID') === 'ubuntu';
}
...
export function activateSearchProvider(prefix = '') {
    if (Me.opt.SEARCH_APP_GRID_MODE && Main.overview.dash.showAppsButton.checked)
        return;
...
// Force redraw of the target widget when source is repainted
// Workaround for visual artifacts caused by repainting of the source widget content
// above a parent widget with added effect
// source, target: Clutter.Actor
export function forwardRepaint(source, target) {
    const override = new Overrides();
    const func = {
        vfunc_paint(...args) {
            Clutter.Actor.prototype.vfunc_paint.bind(source)(...args);
            target.queue_redraw();
        },
    };
    override.addOverride('vfunc_paint', source, func);
    return override;
}

export function fixOverviewIfNeeded() {
    // If overview don't finish hiding the _animationInProgress stays true
    if (Main.overview._shown || !Main.overview._animationInProgress)
        return;

    // print('force close overview');
    Main.overview._shown = true;
    Main.overview._changeShownState('HIDING');
    Main.overview._changeShownState('HIDDEN');
